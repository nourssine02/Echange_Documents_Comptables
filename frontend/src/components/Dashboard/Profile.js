...
const Profile = ({ isSidebarOpen }) => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // Ne mettez à jour que si l'image sélectionnée est différente
        if (base64Image !== formData.profile_image) {
  ...
  const handleSubmit = (e) => {
    ...
    // Clone des données pour éviter de modifier l'état directement
    const updatedData = { ...formData };
  
    // Supprimez `profile_image` si elle n'a pas changé
    if (formData.profile_image === user.profile_image) {
      delete updatedData.profile_image;
    }
  
    axios
      .put(`https://comptaonline.alwaysdata.net/users/${user.id}`, updatedData)
      .then((res) => {
        if (res.data) {
          setUser(res.data); // Met à jour le contexte utilisateur
  
          // Conservez l'aperçu actuel si l'image n'a pas changé
          setFormData({
            ...formData,
            profile_image: res.data.profile_image || formData.profile_image,
          });
          setImagePreview(res.data.profile_image || imagePreview);
          setSuccessMessage("Profile updated successfully");
          setErrorMessage("");
  
          // Effacez le message après 5 secondes
      ...
      .catch((err) => {
        ...
        // Effacez le message d'erreur après 5 secondes
