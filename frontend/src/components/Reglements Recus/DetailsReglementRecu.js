            ...
            >
              <div className="row">
                <div className="col-md-12">
                  <h1 className="card-title" >Détails du Règlement Reçus</h1>
                  <ul className="list-arrow" style={{ fontSize: "14px" }}>
                    <li><strong style={{ color: "darkgreen", fontWeight: "bold" }} >Code Tiers:</strong> {reglement.code_tiers}</li>
                    <li><strong style={{ color: "darkgreen", fontWeight: "bold" }} >Tiers Saisie:</strong> {reglement.tiers_saisie}</li>
                    <li><strong style={{ color: "darkgreen", fontWeight: "bold" }} >Montant Total à Régler:</strong> {reglement.montant_total_a_regler}</li>
                    <li><strong style={{ color: "darkgreen", fontWeight: "bold" }} >Observations:</strong> {reglement.observations}</li>
                  </ul>
                  <h3 className="card-title" >Paiements:</h3>
                  <ul className="list-star" style={{ fontSize: "14px" }}>
                    {payements.map((payement, index) => (
                      <li key={index}>
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} >Modalité:</strong> {payement.modalite}, 
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} > Numéro:</strong> {payement.num}, 
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} > Banque:</strong> {payement.banque}, 
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} > Date d'échéance:</strong> {payement.date_echeance}, 
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} > Montant:</strong> {payement.montant}
                      </li>
                    ))}
                  </ul>
                  ...
                  <ul className="list-star" style={{ fontSize: "14px" }}>
                    {factures.map((facture, index) => (
                      <li key={index}>
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} >Numéro Facture:</strong> {facture.num_facture}, 
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} > Date Facture:</strong> {facture.date_facture}, 
                        <strong style={{ color: "darkgreen", fontWeight: "bold" }} > Montant Total Facture:</strong> {facture.montant_total_facture}, 
