...
function Home({ isSidebarOpen }) {
  ...
  useEffect(() => {
    ...
    const fetchStatistics = async () => {
      try {
        const response = await axios.get("https://comptaonline.alwaysdata.net/statistics");
    ...
    const fetchOrdersPerPeriod = async () => {
      try {
        if (user && user.role === "utilisateur") { // Vérifie si l'utilisateur a le rôle "utilisateur"
          const response = await axios.get("https://comptaonline.alwaysdata.net/orders-per-period");
