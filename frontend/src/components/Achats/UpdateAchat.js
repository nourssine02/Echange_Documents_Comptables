...
const UpdateAchat = ({ isSidebarOpen }) => {
  ...
  const handleClick = async (e) => {
    ...
    try {
      await axios.put(`https://echange-documents-comptables-backend.vercel.app/achats/${id}`, achat);
      Swal.fire({
        icon: "success",
        title: "Succès",
  ...
  useEffect(() => {
    const fetchAchat = async () => {
      try {
        const res = await axios.get(`https://echange-documents-comptables-backend.vercel.app/achats/${id}`);
