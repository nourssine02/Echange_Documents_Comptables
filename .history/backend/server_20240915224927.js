...
app.get("/livraisons-prevues", (req, res) => {
  ...
  // SQL query to get the list of deliveries within the specified date range
  const sql = `
    SELECT date_commande, num_commande, code_tiers, montant_commande, date_livraison_prevue
    FROM commandes
    WHERE date_livraison_prevue BETWEEN ? AND ?
  `;
...
app.get("/factures-non-payees", (req, res) => {
  ...
  let sql = `
    SELECT id, date_facture, montant_total_facture, client, etat_payement
    FROM facturations
