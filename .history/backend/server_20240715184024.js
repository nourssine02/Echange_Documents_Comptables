...
const getBankIdByName = async (bankName) => {
  try {
    ...
    if (rows.length > 0) {
      ...
      throw new Error(`Bank with name ${bankName} not found`);
    }
  } catch (error) {
      ...
      ? Promise.all(
          banques.map(banque => {
            return new Promise((resolve, reject) => {
              const banqueQuery = "SELECT id FROM banques WHERE name = ?";
              db.query(banqueQuery, [banque], (err, results) => {
                if (err) {
                  console.error("Erreur lors de la récupération de l'ID de la banque :", err);
                  return reject(err);
                }
                
                if (results.length === 0) {
                  console.error("Banque non trouvée :", banque);
                  return reject(new Error(`Banque non trouvée: ${banque}`));
                }
                
                const banqueID = results[0].id;
                
                const insertBanqueQuery = "INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)";
                db.query(insertBanqueQuery, [tierID, banqueID], (err, result) => {
                  if (err) {
