...
app.get('/liste-clients-par-magasin', (req, res) => {
  const { magasinId } = req.query;
  const sql = 'SELECT * FROM entreprises WHERE magasin_id = ?';
  db.query(sql, [magasinId], (err, result) => {
    if (err) return res.status(500).send(err);
    res.send(result);
  });
});
...
app.get('/etat-de-facturation', (req, res) => {
  const { startDate, endDate } = req.query;
  const sql = 'SELECT SUM(montant) as totalCA FROM facturations WHERE date_facture BETWEEN ? AND ?';
  db.query(sql, [startDate, endDate], (err, result) => {
    if (err) return res.status(500).send(err);
    res.send(result);
  });
