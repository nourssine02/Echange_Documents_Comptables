...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [identite], (err, results) => {
    ...
    bcrypt.compare(mot_de_passe, results[0].mot_de_passe, (err, isMatch) => {
      ...
      if (isMatch) {
        const token = jwt.sign({ identite, role: results[0].role, id: results[0].id , code_entreprise: results[0].code_entreprise , email: results[0].email }, secretKey, { expiresIn: '7d' });
