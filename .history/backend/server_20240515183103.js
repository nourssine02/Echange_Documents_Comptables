...
function verifyToken(req, res, next) {
  ...
  jwt.verify(token, secretKey, (err, decoded) => {
    if (err) {
      // Si le token est invalide ou expiré, régénérez-le
      if (err.name === 'TokenExpiredError' || err.name === 'JsonWebTokenError') {
        // Extraire l'identifiant de l'utilisateur à partir de la base de données
        db.query('SELECT id FROM utilisateurs WHERE identite = ?', [decoded.identite], (err, results) => {
          if (err || results.length === 0) {
            return res.status(403).json({ message: 'Token invalide' });
          }
          const userId = results[0].id;
          const newToken = jwt.sign({ id: userId }, 'votre_clé_secrète', { expiresIn: '1h' });
          // Ajoutez le nouveau token à l'en-tête de la réponse
          res.setHeader('Authorization', `Bearer ${newToken}`);
          next();
        });
      } else {
        // Si une autre erreur est survenue, renvoyez un message d'erreur
        return res.status(403).json({ message: 'Token invalide' });
      }
    } else {
      // Si le token est valide, passez à l'étape suivante
      req.user = decoded;
      next();
    }
  });
