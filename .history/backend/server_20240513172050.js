...
app.post("/login", (req, res) => {
  ...
  db.query(sql, values, (err, data) => {
    ...
    if (data.length > 0) {
      // Generate JWT token
      jwt.sign({ identite: data[0].identite }, "secret", { expiresIn: "1d" }, (err, token) => {
        if (err) {
          console.error(err);
...
/*************************************************************** */ 
 const secret = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZGVudGl0ZSI6Ik5vdXIiLCJpYXQiOjE3MTU2MTYwMjEsImV4cCI6MTcxNTcwMjQyMX0.39ES6t89QEZb1osN4qUg-_NTV44bNsFc4REC_L0-Mpkvotre_secret";
