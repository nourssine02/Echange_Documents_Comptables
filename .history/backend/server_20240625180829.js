...
const decrypt = (hash) => {
  if (!hash.iv || !hash.content) {
    throw new Error('Invalid hash object: missing iv or content');
  }
  const decipher = crypto.createDecipheriv(algorithm, Buffer.from(secretKey, 'utf-8'), Buffer.from(hash.iv, 'hex'));
...
app.post('/users', (req, res) => {
  ...
  const encryptedPassword = encrypt(mot_de_passe);

  const q = `INSERT INTO utilisateurs (code_entreprise, code_user, identite, position, tel, email, mot_de_passe, password_iv, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  const values = [code_entreprise, code_user, identite, position, tel, email, encryptedPassword.content, encryptedPassword.iv, role];
...
app.put("/users/:id", (req, res) => {
  ...
  db.query(sqlSelect, [id], (err, results) => {
    ...
    let updatedIv = results[0].password_iv;

    if (mot_de_passe) {
      const encryptedPassword = encrypt(mot_de_passe);
      updatedPassword = encryptedPassword.content;
      updatedIv = encryptedPassword.iv;
    }

    const sqlUpdate = "UPDATE utilisateurs SET code_entreprise = ?, code_user = ?, identite = ?, position = ?, tel = ?, email = ?, mot_de_passe = ?, password_iv = ?, role = ? WHERE id = ?";
    const values = [code_entreprise, code_user, identite, position, tel, email, updatedPassword, updatedIv, role, id];
...
app.get("/users/:id", (req, res) => {
  ...
  db.query(q, [userID], (err, data) => {
    ...
    if (data.length > 0) {
      let user = data[0];
      try {
        if (user.password_iv && user.mot_de_passe) {
          const decryptedPassword = decrypt({
            iv: user.password_iv, 
            content: user.mot_de_passe
          });
          user.plain_password = decryptedPassword;
        } else {
