...
const encrypt = (text) => {
  const iv = crypto.randomBytes(16); // Génère un IV de 16 octets
  const cipher = crypto.createCipheriv(algorithm, secretKey, iv);
  let encrypted = cipher.update(text, 'utf8', 'hex');
  encrypted += cipher.final('hex');
  return `${iv.toString('hex')}:${encrypted}`; // Concatène IV et texte chiffré
};
    ...
    bcrypt.compare(mot_de_passe, results[0].mot_de_passe, (err, isMatch) => {
      ...
      if (isMatch) {
        ...
        const refreshToken = jwt.sign(
          { identite, role: results[0].role, id: results[0].id },
          secretKey,
          { expiresIn: "7d" }
        );
...
// Add utilisateur
// Exemple de route pour créer ou mettre à jour un utilisateur
app.post('/users', async (req, res) => {
  const { code_entreprise, code_user, identite, position, tel, email, mot_de_passe, role } = req.body;
  const encryptedPassword = encrypt(mot_de_passe); // Chiffrer le mot de passe

  const sqlInsert = "INSERT INTO utilisateurs (code_entreprise, code_user, identite, position, tel, email, mot_de_passe, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  db.query(sqlInsert, [code_entreprise, code_user, identite, position, tel, email, encryptedPassword, role], (err, results) => {
    if (err) {
...
app.put("/users/:id", async (req, res) => {
  ...
  try {
    ...
    db.query(sqlSelect, [id], async (err, results) => {
      ...
      let plainPassword = results[0].plain_password;

      if (mot_de_passe) {
        updatedPassword = await bcrypt.hash(mot_de_passe, 10);
        plainPassword = mot_de_passe; // Store the plain text password for display
      }

      const sqlUpdate = "UPDATE utilisateurs SET code_entreprise = ?, code_user = ?, identite = ?, position = ?, tel = ?, email = ?, mot_de_passe = ?, plain_password = ?, role = ? WHERE id = ?";
      const values = [code_entreprise, code_user, identite, position, tel, email, updatedPassword, plainPassword, role, id];
...
app.get("/users/:id", async (req, res) => {
  ...
  try {
    const sqlSelect = "SELECT id, code_entreprise, code_user, identite, position, tel, email, mot_de_passe, role FROM utilisateurs WHERE id = ?";
    db.query(sqlSelect, [id], async (err, results) => {
      if (err) {
        console.error("Erreur lors de la récupération de l'utilisateur :", err);
        return res.status(500).json({ error: "Erreur du serveur" });
      ...
      // Déchiffrer le mot de passe si défini
      if (user.mot_de_passe) {
        user.mot_de_passe = decrypt(user.mot_de_passe);
      } else {
        user.mot_de_passe = ''; // Retourne une chaîne vide si mot_de_passe est nul ou vide
      }
...
app.get("/achats", verifyToken, (req, res) => {
  ...
  db.query(
    q,
    [req.user.role === "client" ? req.user.identite : null],
    (err, data) => {
      if (err) return res.json(err);
      return res.json(data);
    }
