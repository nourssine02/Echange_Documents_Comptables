...
app.post('/reglements_emis', verifyToken, (req, res) => {
  ...
  let reglement;
  try {
    reglement = JSON.parse(req.body.reglement);
  } catch (e) {
    console.error('Invalid JSON format:', e);
    return res.status(400).json({ error: 'Invalid JSON format' });
  }

  // Debug: log the reglement object
  console.log('Parsed reglement object:', reglement);

  const { reglement: reglementData, payements, pieces } = reglement;

  // Check if reglementData, payements, and pieces are defined
  if (!reglementData || !payements || !pieces) {
    return res.status(400).json({ error: 'Missing required data in reglement' });
  }

  // Debug: log the reglementData object
  console.log('Reglement data:', reglementData);
  ...
  db.beginTransaction((err) => {
    ...
    // Insert the reglement
    const insertReglementQuery = `
      INSERT INTO reglements_emis 
      (date_saisie, code_tiers, tiers_saisie, montant_brut, base_retenue_source, taux_retenue_source, montant_retenue_source, montant_net, observations, ajoute_par) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

    db.query(insertReglementQuery, [
      reglementData.date_saisie,
      reglementData.code_tiers,
      reglementData.tiers_saisie,
      reglementData.montant_brut,
      reglementData.base_retenue_source,
      reglementData.taux_retenue_source,
      reglementData.montant_retenue_source,
      reglementData.montant_net,
      reglementData.observations,
      userId,
    ], (err, result) => {
      if (err) {
        console.error('Error inserting reglement:', err);
        return db.rollback(() => res.status(500).json({ message: 'Error inserting reglement.' }));
      }
      ...
      const payementPromises = payements.map((payement) =>
        new Promise((resolve, reject) => {
          const insertPayementQuery = `
            INSERT INTO payements (modalite, num, banque, date_echeance, montant, reglement_emis_id) 
            VALUES (?, ?, ?, ?, ?, ?)`;
      ...
      // Insert pieces to pay
      const piecePromises = pieces.map((piece) =>
        new Promise((resolve, reject) => {
          const insertPieceQuery = `
            INSERT INTO pieces_a_regler (num_piece_a_regler, date_piece_a_regler, montant_piece_a_regler, montant_restant, document_fichier, reglement_emis_id) 
            VALUES (?, ?, ?, ?, ?, ?)`;
      ...
      // Execute promises for payments and pieces
      Promise.all([...payementPromises, ...piecePromises])
        .then(() => {
          db.commit((err) => {
            if (err) {
              console.error('Error committing transaction:', err);
              return db.rollback(() => res.status(500).json({ message: 'Error committing transaction.' }));
            }

            // Send notification to accountant
            const notificationMessage = `${req.user.identite} a émis un nouveau règlement`;
            const getComptableQuery = 'SELECT id FROM utilisateurs WHERE role = "comptable"';

            db.query(getComptableQuery, (comptableErr, comptableData) => {
              if (comptableErr) {
                console.error('Error retrieving comptable:', comptableErr);
                return res.status(500).json({ error: 'Error retrieving comptable.' });
              }
              if (comptableData.length === 0) {
                return res.status(404).json({ error: 'No accountant found.' });
              }
              ...
              db.query(notificationQuery, [comptableId, notificationMessage], (notifErr) => {
                ...
                res.status(200).json({ message: 'Reglement issued successfully and notification sent.' });
              });
            });
          });
        })
        .catch((err) => {
          console.error('Error inserting payments or pieces:', err);
          db.rollback(() => res.status(500).json({ message: 'Error inserting payments or pieces.' }));
        });
    });
  });
});
