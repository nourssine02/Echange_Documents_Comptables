...
app.get("/achats/:id", (req, res) => {
  ...
  db.query(q, [achatD], (err, data) => {
    ...
    if (data.length > 0 && data[0].document_fichier) {
      // Convertir le fichier BLOB en Base64
      const fileBuffer = data[0].document_fichier;
      const base64File = fileBuffer.toString('base64');
      
      // Inclure le fichier base64 dans la réponse JSON
      data[0].document_fichier = base64File;
    }
...
app.get("/reglements_emis/:id", async (req, res) => {
  const reglementID = req.params.id;

  try {
    // Récupérer le règlement émis
    const [reglementRows] = await db.query("SELECT * FROM reglements_emis WHERE id = ?", [reglementID]);

    if (reglementRows.length === 0) {
      return res.status(404).json({ message: "Règlement émis non trouvé." });
    }

    const reglement = reglementRows[0];

    // Paralléliser la récupération des payements et pièces associées
    const [payementsRows, piecesRows] = await Promise.all([
      db.query("SELECT * FROM payements WHERE reglement_emis_id = ?", [reglementID]),
      db.query("SELECT * FROM pieces_a_regler WHERE reglement_emis_id = ?", [reglementID])
    ]);

    // S'assurer que ce sont bien des tableaux
    const payements = Array.isArray(payementsRows[0]) ? payementsRows[0] : payementsRows;
    const pieces = Array.isArray(piecesRows[0]) ? piecesRows[0] : piecesRows;

    // Répondre au client avec les données collectées
