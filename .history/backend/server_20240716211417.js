  ...
  db.query(q, values, (err, result) => {
    ...
    addBanques
      .then(() => {
        ...
        const recipientRole = userRole === "comptable" ? "client" : "comptable";

        // Récupérer les utilisateurs du rôle opposé
        const getUsersQuery = "SELECT id FROM utilisateurs WHERE role = ?";
        db.query(getUsersQuery, [recipientRole], (usersErr, usersData) => {
          if (usersErr) {
            console.error(`Erreur lors de la récupération des utilisateurs (${recipientRole}) :`, usersErr);
            return res.status(500).json({ error: `Erreur lors de la récupération des utilisateurs (${recipientRole}).` });
          }
          if (usersData.length === 0) {
            console.error(`Aucun ${recipientRole} trouvé`);
            return res.status(404).json({ error: `Aucun ${recipientRole} trouvé.` });
          }

          const notificationPromises = usersData.map(user => {
            const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
            return new Promise((resolve, reject) => {
              db.query(notificationQuery, [user.id, notificationMessage], (notifErr, notifData) => {
                if (notifErr) {
          ...
          Promise.all(notificationPromises)
            ...
            .catch(notifErr => {
              console.error("Erreur lors de l'ajout des notifications :", notifErr);
              return res.status(500).json({ error: "Erreur lors de l'ajout des notifications." });
            });
        });
      })
      .catch((err) => {
