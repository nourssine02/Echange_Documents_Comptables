...
const addBanqueToTier = async (tierId, banqueName) => {
  try {
    const banqueResult = await axios.get(`http://localhost:5000/banques?name=${banqueName}`);
    const banqueId = banqueResult.data.id;

    if (banqueId) {
      await axios.post('http://localhost:5000/tiers_banques', { tier_id: tierId, banque_id: banqueId });
    } else {
      console.error(`No banque found with name: ${banqueName}`);
    }
  } catch (err) {
    console.error("Error adding banque to tier:", err);
  }
};

// Example usage
addBanqueToTier(1, "AMEN BANK");
