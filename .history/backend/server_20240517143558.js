...
app.post('/documents_comptabilite', (req, res) => {
  const { date, nature, designation, destinataire, document, priorite, observations } = req.body;
  db.query(
    'INSERT INTO documents_comptabilite (date, nature, designation, destinataire, document, priorite, observations) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [date, nature, designation, destinataire, document, priorite, observations],
    (err, results) => {
      if (err) {
        res.status(500).send(err);
      } else {
...
app.put('/documents_comptabilite/:id', (req, res) => {
  ...
  const { date, nature, designation, destinataire, document, priorite, observations } = req.body;
  db.query(
    'UPDATE documents_comptabilite SET date = ?, nature = ?, designation = ?, destinataire = ?, document = ?, priorite = ?, observations = ? WHERE id = ?',
    [date, nature, designation, destinataire, document, priorite, observations, id],
    (err, results) => {
      if (err) {
        res.status(500).send(err);
      } else {
