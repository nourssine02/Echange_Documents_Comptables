...
// Route pour afficher tous les achats avec gestion des rôles
app.get("/achats", authenticateToken, (req, res) => {
  let q = "SELECT a.*, u.role AS ajoute_par, u.client_id FROM achats a LEFT JOIN users u ON a.added_by = u.id";
  let params = [];
  ...
  // Pour le rôle comptable et super_admin, pas de filtre additionnel dans la requête principale

  db.query(q, params, (err, data) => {
    if (err) return res.json(err);

    // Pour le rôle super_admin, inclure des informations supplémentaires
    if (req.user.role === 'super_admin') {
      // Ajouter les informations sur qui a ajouté l'achat et à quel client il appartient
      return res.json(data.map(achat => ({
        ...achat,
        ajoute_par: achat.ajoute_par,
        client_id: achat.client_id,
      })));
    }

    // Pour le rôle comptable, ajouter des informations sur le client
    ...
    // Pour le rôle client, retourner simplement les achats filtrés
    return res.json(data);
  });
});
