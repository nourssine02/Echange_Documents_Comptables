...
// Route pour mettre à jour un tier par ID
router.put('/:id', (req, res) => {
  ...
  const updateTierQuery = `
    UPDATE tiers
    SET code_tiers = ?, date_creation = ?, type = ?, identite = ?, MF_CIN = ?,
        tel = ?, email = ?, adresse = ?, ville = ?, pays = ?, observations = ?, autreType = ?
    WHERE id = ?
  `;

  const updateBanquesQuery = `
  ...
  db.beginTransaction((err) => {
    ...
    db.query(updateTierQuery, [
      ...
      db.query(updateBanquesQuery, [
        id, ...banqueIds.flatMap(banqueId => [id, banqueId])
      ], (err, results) => {
        if (err) {
          return db.rollback(() => {
