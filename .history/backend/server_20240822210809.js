...
app.post("/reglements_emis", verifyToken, (req, res) => {  
  ...
  const reglement = req.body.reglement; // Récupérer l'objet "reglement" envoyé  
    const payements = req.body.payements; // Récupérer les paiements si nécessaire  
    const pieces = req.body.pieces; // Récupérer les pièces si nécessaire  

  // Validation pour les champs requis  
  if (!reglement ||   
      typeof reglement.montant_brut === 'undefined' ||   
      typeof reglement.base_retenue_source === 'undefined' ||   
      typeof reglement.taux_retenue_source === 'undefined' ||   
      typeof reglement.montant_retenue_source === 'undefined' ||   
      typeof reglement.montant_net === 'undefined') {  
    return res.status(400).json({ error: 'Reglement requires montant_brut, base_retenue_source, taux_retenue_source, montant_retenue_source, and montant_net.' });  
  }  
  ...
  db.beginTransaction((err) => {  
    ...
    const insertReglementQuery =  
      "INSERT INTO reglements_emis (date_saisie, code_tiers, tiers_saisie, montant_brut, base_retenue_source , taux_retenue_source, montant_retenue_source , montant_net , observations, ajoute_par) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";  
