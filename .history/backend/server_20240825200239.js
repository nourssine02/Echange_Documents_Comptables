...
app.post("/facture", verifyToken, (req, res) => {
  ...
  if (!facture) {
    console.error("Facture data is missing.");
    return res.status(400).json({ error: "Facture data is required" });
  }

  // Validation des champs de facture
  if (!facture.date_facture || !facture.num_facture) {
    console.error("Facture fields are missing.");
  ...
  db.beginTransaction((err) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Transaction creation error" });
    }

    const insertFactureQuery = `
      INSERT INTO facturations (date_facture, num_facture, code_tiers, tiers_saisie, reference_livraison, montant_HT_facture, FODEC_sur_facture, TVA_facture, timbre_facture, autre_montant_facture, montant_total_facture, observations, document_fichier, etat_payement, ajoute_par)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    db.query(insertFactureQuery, [
      ...
      if (err) {
        console.error("Facture insertion error:", err);
        return db.rollback(() => res.status(500).json({ error: "Facture insertion error" }));
      }

      const getComptableQuery = "SELECT id FROM utilisateurs WHERE role = 'comptable'";
      db.query(getComptableQuery, (err, data) => {
        if (err) {
          console.error("Comptable retrieval error:", err);
          return db.rollback(() => res.status(500).json({ error: "Comptable retrieval error" }));
        }

        if (data.length === 0) {
          console.error("No comptable found.");
          return db.rollback(() => res.status(404).json({ error: "No comptable found" }));
        }
        ...
        db.query(notificationQuery, [comptableId, notificationMessage], (err) => {
          if (err) {
            ...
            return db.rollback(() => res.status(500).json({ error: "Notification insertion error" }));
          }
          ...
          db.commit((err) => {
            ...
            res.status(200).json({ message: "Facture added and notification sent successfully" });
          });
        });
      });
    });
