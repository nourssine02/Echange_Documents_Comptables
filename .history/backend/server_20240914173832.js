...
app.get("/pointage", (req, res) => {
  ...
  db.query("SELECT * FROM pointage_personnel WHERE ajoute_par = ?", [userId], (err, results) => {
    if (err) {
      res.status(500).send(err); // Handle error properly
    } else {
      res.json(results); // Send the query results back
    }
  });
});
