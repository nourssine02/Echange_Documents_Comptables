...
app.post("/achats", upload.single('document_fichier'), verifyToken, (req, res) => {
  try {
    // Check if the file has been received properly
    if (!req.file) {
    ...
    // Ensure the file is correctly received and stored
    const document_fichier = `/uploads/${req.file.filename}`;
    ...
    const values = [
      ...
      montant_total_piece,
      observations,
      document_fichier, // Store the path to the uploaded file
      userId,
    ];
...
app.get("/check-num-piece/:numPiece", async (req, res) => {
  ...
  try {
    // Run the query to check if the piece number exists
    ...
    if (rows && rows.length > 0) {
      ...
      // If rows are empty or undefined, it means no records found
      return res.status(500).json({ error: "No data found" });
    }
  } catch (error) {
    console.error("Server error while checking num_piece:", error);
    return res.status(500).json({ error: "Server error while checking num_piece" });
  }
});
