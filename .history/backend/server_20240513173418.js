...
app.post("/login", (req, res) => {
  ...
  const values = [req.body.identite, req.body.mot_de_passe , req.body.role];
  ...
  db.query(sql, values, (err, data) => {
    ...
    if (data.length > 0) {
      // Generate JWT token
      jwt.sign({ identite: data[0].identite, role: data[0].identite }, secret, { expiresIn: "1d" }, (err, token) => {
        if (err) {
          console.error(err);
