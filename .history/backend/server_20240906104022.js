...
const [payementRows] = await db.query("SELECT * FROM payements WHERE reglement_emis_id = ?", [reglementID]);
const payements = payementRows || [];

// Récupérer les pièces associées
const [pieceRows] = await db.query("SELECT * FROM pieces_a_regler WHERE reglement_emis_id = ?", [reglementID]);
