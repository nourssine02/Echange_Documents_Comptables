...
app.post("/tiers", verifyToken, (req, res) => {
  ...
  db.query(q, values, (err, result) => {
    ...
    const banques = req.body.banques; // Les banques sélectionnées

    if (banques && banques.length > 0) {
      const banqueQueries = banques.map(banque => {
        return new Promise((resolve, reject) => {
          const banqueQuery = "INSERT INTO tiers_banques (tier_id, banque_name) VALUES (?, ?)";
...
app.delete("/tiers/:id", (req, res) => {
  ...
  const checkDependenciesQuery = `
    SELECT COUNT(*) AS count FROM (
      SELECT id FROM achats WHERE tier_id = ?
      UNION
      SELECT id FROM commandes WHERE tier_id = ?
      UNION
      SELECT id FROM livraisons WHERE tier_id = ?
      UNION
      SELECT id FROM facturations WHERE tier_id = ?
      UNION
      SELECT id FROM reglements_emis WHERE tier_id = ?
      UNION
      SELECT id FROM reglements_recus WHERE tier_id = ?
      UNION
      SELECT id FROM pointage_personnel WHERE tier_id = ?
    ) AS dependencies
  `;

  db.query(checkDependenciesQuery, [tierID, tierID, tierID], (err, results) => {
    if (err) return res.status(500).json({ error: "Erreur lors de la vérification des dépendances." });

    const dependenciesCount = results[0].count;

    if (dependenciesCount > 0) {
      return res.status(400).json({ error: "Le tier ne peut pas être supprimé car il a des dépendances." });
    }

    // Si pas de dépendances, supprimer le tier
    const deleteQuery = "DELETE FROM tiers WHERE id = ?";
    db.query(deleteQuery, [tierID], (err, data) => {
      if (err) return res.status(500).json({ error: "Erreur lors de la suppression du tier." });
