...
// Details Achat
app.get('/achats/:id', async (req, res) => {
  ...
  try {
    ...
    db.query(q, [achatID], (err, data) => {
      ...
      if (data.length > 0) {
        const achat = data[0]; // Récupérer les détails de l'achat
        
        // Si document_fichier est un BLOB, on le convertit en base64
        if (achat.document_fichier) {
          achat.document_fichier = Buffer.from(achat.document_fichier).toString('base64');
        }

        // Renvoyer l'objet achat, avec document_fichier converti en base64
    ...
    return res.status(500).json({ error: "Erreur du serveur", details: error });
  }
});
