...
app.put("/tiers/:id", async (req, res) => {
  ...
  try {
    ...
    // Suppression des banques existantes pour le tier
    await db.query('DELETE FROM tiers_banques WHERE tier_id = ?', [tierId]);

    // Insertion des nouvelles banques
...
//   });
// });
app.get("/tiers/:id", async (req, res) => {
  const tierId = req.params.id;
  try {
    const [tierData] = await db.query('SELECT * FROM tiers WHERE id = ?', [tierId]);

    if (tierData.length === 0) {
      return res.status(404).json({ error: 'Tier not found.' });
    }

    const [banquesData] = await db.query(
      'SELECT b.id, b.name FROM banques b JOIN tiers_banques tb ON b.id = tb.banque_id WHERE tb.tier_id = ?',
      [tierId]
    );
    ...
    res.status(500).json({ error: 'Error fetching tier data. Please try again.' });
  }
});
