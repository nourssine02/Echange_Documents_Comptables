...
app.post("/achats", (req, res) => {
  const q =
    "INSERT INTO achats (`date_saisie`, `code_tiers`, `tiers_saisie`, `type_piece`, `num_piece`, `date_piece`, `statut`, `montant_HT_piece`, `FODEC_piece`, `TVA_piece`, `timbre_piece`, `autre_montant_piece`, `montant_total_piece`, `observations`, `document_fichier`, `ajoute_par `,`clientId`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  ...
  const values = [
    ...
    req.body.clientId, // Ajout de l'ID du client
    req
  ];

  // Vérifier les valeurs vides et les remplacer par null
  ...
  db.query(q, sanitizedValues, (err, data) => {
    if (err) {
     ...
     const notificationMessage = `${req.body.clientId} modif table achats`;
     const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
     db.query(notificationQuery, [req.body.clientId, notificationMessage], (notifErr, notifData) => {
       if (notifErr) {
