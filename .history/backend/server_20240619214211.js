...
function handleDisconnect() {
  connection = mysql.createConnection({
    host: 'localhost', // Remplacez par les détails de votre serveur MySQL
...
app.post('/refresh-token', (req, res) => {
  ...
  jwt.verify(refreshToken, refreshTokenKey, (err, decoded) => {
      ...
      const newToken = jwt.sign({ id: decoded.id, identite: decoded.identite, role: decoded.role }, secretKey, {
...
app.post('/reset-password', (req, res) => {
  ...
  db.query(
    ...
    [token, Date.now()],
    (err, results) => {
      if (err || results.length === 0) return res.status(400).send('Password reset token is invalid or has expired.');
...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [identite], (err, results) => {
    ...
    bcrypt.compare(mot_de_passe, results[0].mot_de_passe, (err, isMatch) => {
      ...
      if (isMatch) {
        const token = jwt.sign({ identite, role: results[0].role, id: results[0].id , code_entreprise: results[0].code_entreprise }, secretKey, { expiresIn: '7d' });
        const refreshToken = jwt.sign({ identite, role: results[0].role, id: results[0].id }, secretKey, { expiresIn: '7d' });
        res.json({ auth: true, token: token, refreshToken: refreshToken });
      } else {
...
app.post('/taux_retenue_source', (req, res) => {
  const { taux } = req.body;
  db.query('INSERT INTO taux_retenue_source (taux, active) VALUES (?, true)', [taux], (err, results) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.send(results);
...
app.put('/taux_retenue_source/:id', (req, res) => {
  ...
  db.query('UPDATE taux_retenue_source SET active = ? WHERE id = ?', [active, id], (err, results) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.send(results);
...
app.put('/taux_retenue_source/modif/:id', (req, res) => {
  const { id } = req.params;
  const { taux } = req.body;
  db.query('UPDATE taux_retenue_source SET taux = ? WHERE id = ?', [taux, id], (err, results) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.send(results);
...
app.get('/taux_retenue_source/active', (req, res) => {
  db.query('SELECT `taux` FROM `taux_retenue_source` WHERE `active` = 1', (err, results) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.send(results);
...
app.get("/achats", verifyToken, (req, res) => {
  ...
  // Appliquer des filtres basés sur le rôle de l'utilisateur
  if (req.user.role === 'client') {
    q += " WHERE u.identite = ?";
  }

  db.query(q, [req.user.role === 'client' ? req.user.identite : null], (err, data) => {
    if (err) return res.json(err);
    return res.json(data);
  });
});
...
app.post("/achats", verifyToken, (req, res) => {
  ...
  db.query(q, values, (err, data) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Failed to add achat to database", details: err.message });
    }
    ...
    db.query(getComptableQuery, (comptableErr, comptableData) => {
      if (comptableErr) {
        ...
        return res.status(500).json({ error: "Failed to fetch comptable", details: comptableErr.message });
      }
      ...
      db.query(notificationQuery, [comptableId, notificationMessage], (notifErr, notifData) => {
        if (notifErr) {
          ...
          return res.status(500).json({ error: "Failed to add notification", details: notifErr.message });
        }
  ...
  db.beginTransaction((err) => {
    ...
    db.query(insertReglementQuery, [
      ...
      if (err) {
        ...
        return db.rollback(() => res.status(500).json({ message: "Erreur lors de l'insertion du règlement émis." }));
      }

      const reglementId = result.insertId;

      const payementPromises = payements.map((payement) => new Promise((resolve, reject) => {
        const insertPayementQuery =
          "INSERT INTO payements (modalite, num, banque, date_echeance, montant, reglement_emis_id) VALUES (?, ?, ?, ?, ?, ?)";
        db.query(insertPayementQuery, [
          payement.modalite,
          payement.num,
          payement.banque,
      ...
      const piecePromises = pieces.map((piece) => new Promise((resolve, reject) => {
          ...
          db.commit((err) => {
            ...
            db.query(getComptableQuery, (comptableErr, comptableData) => {
              ...
              db.query(notificationQuery, [comptableId, notificationMessage], (notifErr, notifData) => {
                ...
                return res.status(200).json({ message: "Règlement émis ajouté avec succès et notification envoyée" });
              });
            });
          });
        })
        .catch((err) => {
          console.error("Erreur lors de l'insertion des payements ou pièces:", err);
          db.rollback(() => res.status(500).json({ message: "Erreur lors de l'insertion des payements ou pièces." }));
        });
    });
  });
});
  ...
  db.beginTransaction((err) => {
    ...
    db.query(insertCommandeQuery, [
      ...
      if (err) {
        ...
        return db.rollback(() => res.status(500).json({ message: "Erreur lors de l'insertion de la commande." }));
      }

      const commandeId = result.insertId;

      const famillePromises = familles.map((famille) => new Promise((resolve, reject) => {
        const insertFamilleQuery =
          "INSERT INTO `familles`(`famille`, `sous_famille`, `article`, `quantite`, `prix`, `commande_id`) VALUES (?,?,?,?,?,?)";
        db.query(insertFamilleQuery, [
          famille.famille,
      ...
      Promise.all([...famillePromises])
          ...
          db.commit((err) => {
            ...
            db.query(getComptableQuery, (comptableErr, comptableData) => {
              ...
              db.query(notificationQuery, [comptableId, notificationMessage], (notifErr, notifData) => {
                ...
                return res.status(200).json({ message: "Commande ajoutée avec succès et notification envoyée" });
              });
            });
          });
        })
        .catch((err) => {
          ...
          db.rollback(() => res.status(500).json({ message: "Erreur lors de l'insertion des familles." }));
        });
    });
  });
});
  ...
  db.query(q, [values], (err, data) => {
    ...
    db.query(getComptableQuery, (comptableErr, comptableData) => {
      ...
      db.query(notificationQuery, [comptableId, notificationMessage], (notifErr, notifData) => {
        ...
        return res.status(200).json({ message: "Livraison ajoutée avec succès et notification envoyée" });
      });
    });
  });
});
...
app.get("/familles/:famille", (req, res) => {
  ...
  const sql =
    "SELECT famille, sous_famille, article, quantite, prix FROM familles WHERE famille = ?";
...
// // Route pour ajouter une nouvelle famille
// app.post("/familles", (req, res) => {
//   const famille = req.body.famille;
//   const sql = "INSERT INTO `familles`(`famille`, `sous_famille`, `article`, `quantite`,`prix`)";
//   db.query(sql, famille, (err, result) => {
//     if (err) {
//       throw err;
//     }
//     console.log("Nouvelle famille ajoutée :", famille);
//     res.send("Nouvelle famille ajoutée avec succès");
//   });
// });
...
app.get('/articles', (req, res) => {
  const query =  "SELECT article, quantite, prix FROM familles";
    ...
    db.query(
      ...
      (err, result) => {
        ...
        const famillePromises = familles.map((famille) => {
          return new Promise((resolve, reject) => {
            const insertFamilleQuery =
              "INSERT INTO `familles`(`famille`, `sous_famille`, `article`, `quantite`, `prix`, `facture_id`) VALUES (?,?,?,?,?,?)";
            db.query(
              insertFamilleQuery,
              [
                famille.famille,
...
app.post('/pointage', (req, res) => {
  ...
  let sql = 'INSERT INTO pointage_personnel (date_de_saisie, code_tiers, nbre_jours_travailles1, nbre_jours_travailles2, nbre_jours_absence1, nbre_jours_absence2, nbre_jours_conges1, nbre_jours_conges2, supplement_recus, sommes_rejetees, remboursement_divers, autre_deduction, observations) VALUES ?';
...
app.post('/documents_direction', (req, res) => {
  const { date, nature, designation, destinataire, document_fichier, priorite, observations } = req.body;
  const query = 'INSERT INTO documents_direction (date, nature, designation, destinataire, document_fichier, priorite, observations) VALUES (?, ?, ?, ?, ?, ?, ?)';
  
  db.query(query, [date, nature, designation, destinataire, document_fichier, priorite, observations], (err, results) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.status(201).send('Document créé');
...
app.put('/documents_direction/:id', (req, res) => {
  const { id } = req.params;
  const { date, nature, designation, destinataire, document_fichier, priorite, observations } = req.body;
  const query = 'UPDATE documents_direction SET date = ?, nature = ?, designation = ?, destinataire = ?, document_fichier = ?, priorite = ?, observations = ? WHERE id = ?';
  
  db.query(query, [date, nature, designation, destinataire, document_fichier, priorite, observations, id], (err, results) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.send('Document modifié');
