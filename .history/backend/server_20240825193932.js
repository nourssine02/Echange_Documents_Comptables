...
const upload = multer({
  limits: { fileSize: 5 * 1024 * 1024 }, // Limite de taille de fichier (5 MB)
});
        ...
        Promise.all([...payementPromises, ...piecePromises])
          .then(() => {
            db.commit((err) => {
              ...
              const notificationMessage = `${req.user.identite} a ajouté un nouveau règlement`;
...
app.post("/facture", verifyToken, upload.single("document_fichier"), async (req, res) => {
  const userId = req.user.id; // Utilisateur qui ajoute la facture
  if (!userId) {
    return res.status(400).json({ error: "User ID is required to add facture" });
  }

  const { facture } = JSON.parse(req.body.facture); // Parsing des données JSON envoyées avec le fichier
  if (!facture) {
    return res.status(400).json({ error: "Facture data is required" });
  }

  const notificationMessage = `${req.user.identite} a ajouté une nouvelle facture`;
  ...
  try {
    ...
    const insertFactureQuery =
      "INSERT INTO `facturations`(`date_facture`, `num_facture`, `code_tiers`, `tiers_saisie`, `reference_livraison`, `montant_HT_facture`, `FODEC_sur_facture`, `TVA_facture`, `timbre_facture`, `autre_montant_facture`, `montant_total_facture`, `observations`, `document_fichier`, `etat_payement`, `ajoute_par`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    const result = await db.query(insertFactureQuery, [
      ...
      req.file.buffer, // Le fichier est stocké en binaire
      facture.etat_payement,
      userId,
    ]);

    // 2. Récupérer le comptable
    const [comptableData] = await db.query("SELECT id FROM utilisateurs WHERE role = 'comptable'");

    if (comptableData.length === 0) {
      throw new Error("Aucun comptable trouvé");
    }

    const comptableId = comptableData[0].id;

    // 3. Ajouter la notification pour le comptable
    ...
    await db.commit();
    res.status(200).json({ message: "Facture insérée et notification envoyée avec succès." });
  } catch (err) {
    await db.rollback();
    console.error("Erreur lors de la transaction :", err);
    res.status(500).json({ message: "Erreur lors de l'ajout de la facture.", details: err.message });
  }
});
// app.post("/facture", verifyToken, (req, res) => {
//   const userId = req.user.id; // L'utilisateur qui ajoute la facture
//   if (!userId) {
//     console.error("User ID is undefined. Cannot proceed with adding facture.");
//     return res.status(400).json({ error: "User ID is required to add facture" });
//   }

//   const { facture } = req.body;
  
//   // Vérifier si l'objet facture existe
//   if (!facture) {
//     console.error("Facture data is undefined. Cannot proceed.");
//     return res.status(400).json({ error: "Facture data is required" });
//   }
...
//           if (comptableErr) {
//             console.error("Erreur lors de la récupération du comptable :", comptableErr);
//             // Annuler la transaction si la récupération échoue
//             return db.rollback(() => {
//               return res.status(500).json({
...
//           if (comptableData.length === 0) {
//             console.error("Aucun comptable trouvé");
//             // Annuler la transaction si aucun comptable n'est trouvé
//             return db.rollback(() => {
//               return res.status(404).json({ error: "Aucun comptable trouvé" });
//             });
//           }
...
//                 return db.rollback(() => {
//                   return res.status(500).json({
//                     message: "Erreur lors de l'ajout de la notification",
//                     details: notifErr.message,
//                   });
//                 });
//               }
...
//                 // Réponse positive après l'insertion réussie et notification envoyée
//                 res.status(200).json({
//                   message: "Facture insérée et notification envoyée avec succès.",
//                 });
//               });
//             }
//           );
