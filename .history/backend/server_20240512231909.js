...
function verifyToken(req, res, next) {
  ...
  jwt.verify(jwtToken, "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZGVudGl0ZSI6IkNvbXB0YSIsImlhdCI6MTcxNTU1MjMwMywiZXhwIjoxNzE1NjM4NzAzfQ.StDtkW7_WCr701Jo3obqZXthUSbJiEfpDn5HTHVlnnc", (err, decoded) => {
    if (err) return res.status(403).json({ message: "Invalid token" });

    // Assurez-vous que le décodage contient les propriétés identite et role
    if (!decoded.identite || !decoded.role) {
      return res.status(403).json({ message: "Incomplete user information" });
    }

    // Ajouter les informations d'identité et de rôle de l'utilisateur à l'objet req.user
    req.user = {
...
app.put('/reglements_recus/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Le règlement reçu a été mis à jour avec succès.' });
  } catch (error) {
    // Rollback transaction in case of error
    await db.rollback();
    console.error('Erreur lors de la mise à jour du règlement reçu :', error);
    res.status(500).json({ error: 'Une erreur s\'est produite lors de la mise à jour du règlement reçu.' });
  }
});
...
app.post('/versement', (req, res) => {
  ...
  db.query('INSERT INTO versements_en_banque SET ?', versement, (err, result) => {
    if (err) {
    ...
    const payementsValues = payements.map(payement => [payement.modalite, payement.num, payement.banque, payement.date_echeance, payement.montant, payement.code_tiers, payement.tiers_saisie, versementId]);

    // Insérer les payements dans la table payements
    db.query('INSERT INTO payements (modalite, num, banque, date_echeance, montant, code_tiers, tiers_saisie, versement_id) VALUES ?', [payementsValues], (err, result) => {
      if (err) {
...
app.put("/versement/:id", async (req, res) => {
  ...
  try {
    ...
    // Update received payment data
    const updateVersementQuery = "UPDATE `versements_en_banque` SET `date_versement`=?,`reference_bordereau_bulletin`=?,`observations`=?,`document_fichier`=? WHERE id = ?";
    ...
    res.status(200).json({ message: 'Le Versement a été mis à jour avec succès.' });
  } catch (error) {
    // Rollback transaction in case of error
    await db.rollback();
    console.error('Erreur lors de la mise à jour du Versement :', error);
    res.status(500).json({ error: 'Une erreur s\'est produite lors de la mise à jour du Versement.' });
  }
});
