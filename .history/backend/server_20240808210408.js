...
const OCR_SPACE_API_KEY = "K83177635088957";
...
app.post('/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    // Appel à l'API OCR pour extraire le texte
    ...
    // Appel à l'API ChatGPT pour structurer les données
    const chatGptResponse = await axios.post(
      'https://api.openai.com/v1/engines/gpt-3.5-turbo/completions',
      {
        prompt: `Extract the following details from the text: Date, Invoice Number, HT Amount, FODEC, VAT, Stamp, Total Amount. Text: ${extractedText}`,
        max_tokens: 150,
      },
      {
        headers: {
    ...
    const data = chatGptResponse.data.choices[0].text.trim().split('\n').reduce((acc, line) => {
    ...
    const sql = 'INSERT INTO invoices (file_url, date_facture, num_facture, montant_HT_facture, FODEC_sur_facture, TVA_facture, timbre_facture, montant_total_facture) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
    const values = [
      fileUrl,
      data.Date || '',
      data['Invoice Number'] || '',
    ...
    db.query(sql, values, (err, results) => {
      ...
      console.log('Data stored successfully:', results);
      res.send('File uploaded and data processed successfully.');
    });
  } catch (error) {
    console.error('Error during OCR and analysis:', error);
    res.status(500).send('Error during OCR and analysis.');
  }
});

// Route pour gérer le téléchargement de fichiers et l'extraction de texte
// app.post("/upload", upload.single("file"), async (req, res) => {
//   const file = req.file;

//   // Ajoutez ce log pour vérifier que le fichier a bien été téléchargé
...
//     const {
//       data: { text },
//     } = await Tesseract.recognize(file.path, "fra", {
//       logger: (m) => console.log(m),
//     });

//     // Utilisez l'API OpenAI pour structurer les données
...
//           {
//             role: "user",
//             content: `Extract the following details from the invoice text:\n
//                   - Date de facture\n
//                   - Numéro de facture\n
//                   - Montant HT\n
//                   - FODEC\n
...
//     res.status(500).send("Erreur lors de l'analyse de la facture.");
//   }
// });
