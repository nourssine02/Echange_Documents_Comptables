...
app.get("/tiers/:id", (req, res) => {
  const tierID = req.params.id;
  const q = `
    SELECT tiers.*, GROUP_CONCAT(banque.nom_banque SEPARATOR ', ') AS banques
    FROM tiers
    LEFT JOIN tiers_banques ON tiers.id = tiers_banques.id_tiers
    LEFT JOIN banque ON tiers_banques.id_banque = banque.id
    WHERE tiers.id = ?
    GROUP BY tiers.id
  `;
