  ...
  try {
    ...
    if (Array.isArray(banques) && banques.length > 0) {
      for (const banque of banques) {
        // Retrieve the bank ID by bank name
        const [results] = await db.query('SELECT id FROM banques WHERE name = ?', [banque.label]);

        console.log(`Results for bank ${banque.label}:`, results); // Debugging line

        if (Array.isArray(results) && results.length > 0) {
          const banqueId = results[0].id;
          await db.query('INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)', [tierId, banqueId]);
        } else {
          throw new Error(`Bank with name ${banque.label} does not exist`);
        }
      }
    }
