...
app.post("/achats", verifyToken, (req, res) => {
  ...
  // Validate input
  if (!date_saisie || !code_tiers || !type_piece || !statut || !montant_HT_piece || !clientId) {
    return res.status(400).json({ error: "Missing required fields for achat" });
  }
  ...
  db.query(q, values, (err, data) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Failed to add achat" });
    }

    // Add notification
    const notificationMessage = `${userId} added a achat for client ${clientId}`;
    const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
