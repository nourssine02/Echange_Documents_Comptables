...
// Middleware pour traiter les données JSON avec une limite de taille plus élevée
...
app.post('/refresh-token', (req, res) => {
  ...
  jwt.verify(token, secretKey, (err, decoded) => {
    ...
    const newToken = jwt.sign({ identite: decoded.identite, role: decoded.role, id: decoded.id }, secretKey, { expiresIn: '1d' });
...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [identite], (err, results) => {
    ...
    bcrypt.compare(mot_de_passe, results[0].mot_de_passe, (err, isMatch) => {
      ...
      if (isMatch) {
        const token = jwt.sign({ identite, role: results[0].role, id: results[0].id , code_entreprise: results[0].code_entreprise }, secretKey, { expiresIn: '1d' });
...
app.post('/pointage', (req, res) => {
  ...
  let sql = 'INSERT INTO salary_data (date_de_saisie, code_tiers, nbre_jours_travailles1, nbre_jours_travailles2, nbre_jours_absence1, nbre_jours_absence2, nbre_jours_conges1, nbre_jours_conges2, supplement_recus, sommes_rejetees, remboursement_divers, autre_deduction, observations) VALUES ?';
