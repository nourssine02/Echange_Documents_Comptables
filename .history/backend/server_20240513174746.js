...
app.post('/login', (req, res) => {
  ...
  connection.query(sql, [identite, mot_de_passe], (err, results) => {
    if (err) throw err;
    if (results.length > 0) {
      // Création du token JWT avec l'identité et le rôle
      const token = jwt.sign({ identite: results[0].identite, role: results[0].role }, 'secret', { expiresIn: '1h' });
      res.json({ token });
    } else {
...
function verifyToken(req, res, next) {
  ...
  jwt.verify(token, secret, (err, decoded) => {
    ...
    // Ajouter les informations d'identité et de rôle à l'objet req.user
    req.user = {
...
app.get('/authenticated', verifyToken, (req, res) => {
  res.json({ identite: req.user.identite, role: req.user.role });
});
