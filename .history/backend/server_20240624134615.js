...
// Get user by ID with plain password (for demonstration purposes only)
app.get("/users/:id", async (req, res) => {
  const { id } = req.params;

  try {
    const sqlSelect = "SELECT id, code_entreprise, code_user, identite, position, tel, email, plain_password AS mot_de_passe, role FROM utilisateurs WHERE id = ?";
    db.query(sqlSelect, [id], async (err, results) => {
      if (err) {
        console.error("Erreur lors de la récupération de l'utilisateur :", err);
        return res.status(500).json({ error: "Erreur du serveur" });
