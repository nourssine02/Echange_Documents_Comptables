...
app.post('/register', async (req, res) => {
    console.log('Requête POST reçue'); // Ajout d'un log ici pour confirmer que la route est atteinte.
  ...
  `;
    ...
    try {
        // Convertir db.query() en promesse pour pouvoir utiliser await
        await new Promise((resolve, reject) => {
            db.query(
                ...
                (err, result) => {
                    if (err) {
                        console.error('Erreur lors de la requête SQL:', err); // Ajoutez un log ici
                        reject(err);
                    } else {
                        resolve(result);
                    }
