  ...
  db.beginTransaction((err) => {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        // Confirmer la transaction si l'insertion réussit
        db.commit((err) => {
          if (err) {
            ...
            return db.rollback(() => {
              return res
                .status(500)
                .json({ message: "Erreur lors de la confirmation de la transaction." });
            });
          }

          // Réponse positive après l'insertion réussie
          res.status(200).json({ message: "Facture insérée avec succès." });
        });
      }
    );
  });
