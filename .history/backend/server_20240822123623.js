...
app.post('/reglements_emis', verifyToken, (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(insertReglementQuery, [
      ...
      // Exécution des promesses d'insertion
      Promise.all([...payementPromises, ...piecePromises])
        .then(() => {
          db.commit((err) => {
            ...
            // Envoi de la notification au comptable
        ...
        .catch((err) => {
          db.rollback(() => res.status(500).json({ message: 'Erreur lors de l\'insertion des paiements ou des pièces.' }));
        });
    });
  });
});
