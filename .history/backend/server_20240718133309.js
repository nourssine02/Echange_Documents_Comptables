...
//     // Clear existing banks for the tier
//     await db.query('DELETE FROM tiers_banques WHERE tier_id = ?', [tierId]);

//     // Insert new banks
//     if (Array.isArray(banques) && banques.length > 0) {
//       for (const banqueId of banques) {
//         await db.query('INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)', [tierId, banqueId]);
//       }
//     }
...
//     res.status(500).json({ error: 'Error updating tier. Please try again.' });
//   }
// });
app.put("/tiers/:id", async (req, res) => {
  const tierId = req.params.id;
  ...
  try {
    ...
    // Update the main 'tiers' table
    await db.query(
      'UPDATE tiers SET code_tiers = ?, date_creation = ?, type = ?, autreType = ?, identite = ?, `MF/CIN` = ?, tel = ?, email = ?, adresse = ?, ville = ?, pays = ?, observations = ? WHERE id = ?',
      [code_tiers, date_creation, type, autreType, identite, MF_CIN, tel, email, adresse, ville, pays, observations, tierId]
    );

    // Validate the bank IDs before proceeding
    const validBankIds = (await db.query('SELECT id FROM banques WHERE id IN (?)', [banques])).map(row => row.id);
    if (validBankIds.length !== banques.length) {
      throw new Error('Invalid banque_id(s) provided');
    }
    ...
    if (Array.isArray(banques) && banques.length > 0) {
      for (const banque of banques) {
        // Ensure banque.value is the ID of the bank
        await db.query('INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)', [tierId, banque.value]);
      }
    }
