...
const refreshToken = "my-refresh-token-secret-key";
...
//     const newToken = jwt.sign({ identite: decoded.identite, role: decoded.role, id: decoded.id }, secretKey, { expiresIn: '7d' });
...
app.post('/refresh-token', (req, res) => {
  ...
  jwt.verify(refreshToken, 'your-refresh-token-secret-key', (err, decoded) => {
      if (err) {
          return res.status(401).send({ message: 'Unauthorized!' });
      }

      const newToken = jwt.sign({ id: decoded.id }, 'your-secret-key', {
