...
app.post('/users', async (req, res) => {
  const { code_entreprise, code_user, identite, position, tel, email, mot_de_passe, role } = req.body;
  ...
  try {
    ...
    const values = [code_entreprise, code_user, identite, position, tel, email, encryptedPassword, role];
...
app.put("/users/:id", async (req, res) => {
  ...
  db.query(sqlSelect, [id], async (err, results) => {
    ...
    const sqlUpdate = "UPDATE utilisateurs SET code_entreprise = ?, code_user = ?, identite = ?, position = ?, tel = ?, email = ?, mot_de_passe = ?, role = ? WHERE id = ?";
    const values = [code_entreprise, code_user, identite, position, tel, email, updatedPassword, role, id];
...
app.get('/banques/id', async (req, res) => {
  ...
  try {
    const query = "SELECT id FROM banques WHERE name = ? OR label = ?";
    const [rows] = await db.promise().query(query, [bankName, bankName]);
    if (rows.length > 0) {
      res.json({ id: rows[0].id });
    } else {
      res.status(404).json({ error: `Bank with name ${bankName} not found` });
    }
  } catch (error) {
...
//   const q = `
//     INSERT INTO tiers (
//       code_tiers, date_creation, type, identite, \`MF/CIN\`, tel, email, adresse, ville, pays, observations
//     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
//   `;
...
//         return db.rollback(() => {
//           res.status(500).json({ error: "Erreur lors de l'ajout du tier." });
//         });
//       }

//       const tierID = result.insertId;
//       const banques = req.body.banques;

//       const addBanques = banques && banques.length > 0
//         ? Promise.all(
//             banques.map(banque => {
//               return new Promise((resolve, reject) => {
//                 const banqueQuery = "INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)";
...
//               db.query(notificationQuery, [userId, message], (notifErr, notifData) => {
//                 if (notifErr) {
...
//             if (comptableErr) {
//               console.error("Erreur lors de la récupération du comptable :", comptableErr);
//               return db.rollback(() => {
//                 res.status(500).json({ error: "Erreur lors de la récupération du comptable." });
//               });
//             }
...
//             const notificationMessage = `${req.user.identite} a ajouté un Tier`;

//             addNotification(comptableId, notificationMessage)
//               .then(() => {
//                 // Récupérer le client
//                 const getClientQuery = "SELECT id FROM utilisateurs WHERE role = 'client'";
//                 db.query(getClientQuery, (clientErr, clientData) => {
//                   if (clientErr) {
//                     console.error("Erreur lors de la récupération du client :", clientErr);
//                     return db.rollback(() => {
...
//                   addNotification(clientId, notificationMessage)
//                     .then(() => {
//                       db.commit((commitErr) => {
//                         if (commitErr) {
...
//                         res.status(200).json({ message: "Le tier, les banques et les notifications ont été ajoutés avec succès." });
//                       });
//                     })
//                     .catch((notifClientErr) => {
//                       console.error("Erreur lors de l'ajout de la notification pour le client :", notifClientErr);
//                       db.rollback(() => {
//                         res.status(500).json({ error: "Erreur lors de l'ajout de la notification pour le client." });
//                       });
//                     });
//                 });
//               })
//               .catch((notifErr) => {
//                 console.error("Erreur lors de l'ajout de la notification pour le comptable :", notifErr);
//                 db.rollback(() => {
//                   res.status(500).json({ error: "Erreur lors de l'ajout de la notification pour le comptable." });
//                 });
//               });
//           });
//         })
...
//           db.rollback(() => {
//             res.status(500).json({ error: "Erreur lors de l'ajout des banques du tier." });
//           });
//         });
//     });
//   });
    ...
    db.query(q, values, (err, result) => {
      ...
      addBanques
        .then(() => {
          ...
          // Récupérer l'ID de l'utilisateur à notifier en fonction du rôle
          const recipientRole = userRole === 'client' ? 'comptable' : 'client';

          const getRecipientQuery = "SELECT id FROM utilisateurs WHERE role = ?";
          db.query(getRecipientQuery, [recipientRole], (recipientErr, recipientData) => {
            if (recipientErr) {
              console.error(`Erreur lors de la récupération du ${recipientRole} pour la notification :`, recipientErr);
              return db.rollback(() => {
                res.status(500).json({ error: `Erreur lors de la récupération du ${recipientRole} pour la notification.` });
              });
            }
            if (recipientData.length === 0) {
              console.error(`Aucun ${recipientRole} trouvé pour la notification.`);
              return db.rollback(() => {
                res.status(404).json({ error: `Aucun ${recipientRole} trouvé pour la notification.` });
              });
            }
            ...
            // Ajouter la notification au destinataire approprié
            addNotification(recipientId, notificationMessage)
              .then(() => {
                // Committre la transaction après avoir ajouté la notification
                db.commit((commitErr) => {
                  if (commitErr) {
                    console.error("Erreur lors de la validation de la transaction :", commitErr);
                    return db.rollback(() => {
              ...
              .catch((notifErr) => {
                console.error(`Erreur lors de l'ajout de la notification pour le ${recipientRole} :`, notifErr);
                db.rollback(() => {
                  res.status(500).json({ error: `Erreur lors de l'ajout de la notification pour le ${recipientRole}.` });
                });
              });
          });
        })
...
// Update tier
app.put("/tiers/:id", verifyToken, (req, res) => {
  const tierID = req.params.id;
  const q = `
    UPDATE tiers 
    SET code_tiers = ?, date_creation = ?, type = ?, identite = ?, \`MF/CIN\` = ?, tel = ?, email = ?, adresse = ?, ville = ?, pays = ?, observations = ?
    WHERE id = ?
  `;
  ...
  db.query(q, values, (err, data) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Erreur lors de la mise à jour du tier." });
    }

    const deleteBanqueQuery = "DELETE FROM tiers_banques WHERE tier_id = ?";
    db.query(deleteBanqueQuery, [tierID], (err, result) => {
      if (err) {
        console.error("Erreur lors de la suppression des banques existantes du tier :", err);
        return res.status(500).json({ error: "Erreur lors de la suppression des banques existantes du tier." });
      }

      const banques = req.body.banques;

      if (banques && banques.length > 0) {
        const banqueQueries = banques.map(banque => {
          return new Promise((resolve, reject) => {
            const banqueQuery = "INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)";
            db.query(banqueQuery, [tierID, banque.value], (err, result) => {
              if (err) {
        ...
        Promise.all(banqueQueries)
          .then(() => {
            return res.status(200).json({ message: "Le tier et les banques ont été mis à jour avec succès." });
          })
          .catch((err) => {
            console.error("Erreur lors de l'ajout des banques du tier :", err);
            return res.status(500).json({ error: "Erreur lors de l'ajout des banques du tier." });
          });
      } else {
        return res.status(200).json({ message: "Le tier a été mis à jour avec succès." });
      }
    });
  });
});
...
app.get("/tiers/:id", (req, res) => {
  ...
  db.query(q, [tierID], (err, data) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Erreur lors de la récupération du tier et des banques associées." });
    }
    if (data.length === 0) {
      return res.status(404).json({ error: "Tier non trouvé." });
    }
    // Modify response structure if needed before sending
    const tierData = {
      id: data[0].id,
      ...
      banques: data[0].banques.split(', '), // Assuming banques field is comma-separated
    };
    res.json(tierData);
  });
});
...
app.delete("/tiers/:id", (req, res) => {
  ...
  db.query(checkDependenciesQuery, [tierID, tierID, tierID, tierID, tierID, tierID, tierID], (err, results) => {
    if (err) {
    ...
    if (dependenciesCount > 0) {
      ...
      const dependentTables = tablesWithDependencies.filter(table => results[0][table.table] > 0);

      // Construire un message détaillé pour l'utilisateur
      const dependentTablesMessage = dependentTables.map(table => table.label).join(', ');
...
app.get("/achats", verifyToken, (req, res) => {
  ...
  if (req.user.role === "client") {
    ...
    db.query(q, [req.user.identite], (err, data) => {
      if (err) {
        return res.status(500).json({ error: "Erreur lors de la récupération des achats du client" });
      }
      return res.json(data);
    });
    ...
    if (clientId) {
      ...
      db.query(q, [clientId], (err, data) => {
        if (err) {
          return res.status(500).json({ error: "Erreur lors de la récupération des achats du client spécifié" });
        }
        return res.json(data);
      });
    } else {
      db.query(q, [], (err, data) => {
        if (err) {
          return res.status(500).json({ error: "Erreur lors de la récupération de tous les achats pour les comptables" });
        }
        return res.json(data);
      });
    }
          ...
          (famille) =>
            new Promise((resolve, reject) => {
              const insertFamilleQuery =
                "INSERT INTO `familles`(`famille`, `sous_famille`, `article`, `quantite`, `prix`, `TVA` , `prix_total`, `commande_id`) VALUES (?,?,?,?,?,?,?,?)";
              db.query(
                insertFamilleQuery,
                [
                  famille.famille,
...
//           return new Promise((resolve, reject) => {
//             const insertFamilleQuery =
//               "INSERT INTO `familles`(`famille`, `sous_famille`, `article`, `quantite`, `prix`, `commande_id`) VALUES (?,?,?,?,?,?)";
//             db.query(
//               insertFamilleQuery,
//               [
//                 famille.famille,
...
app.get("/familles/:famille", (req, res) => {
  ...
  const sql =
    "SELECT famille, sous_famille, article, quantite, prix , TVA, prix_total FROM familles WHERE famille = ?";
...
app.get("/articles", (req, res) => {
  const query = "SELECT DISTINCT article, quantite, prix_total FROM familles";
    ...
    db.query(
      ...
      (err, result) => {
        ...
        const famillePromises = familles.map((famille) => {
          return new Promise((resolve, reject) => {
            const insertFamilleQuery =
              "INSERT INTO `familles`(`famille`, `sous_famille`, `article`, `quantite`, `prix`, `TVA` , `prix_total`, `facture_id`) VALUES (?,?,?,?,?,?,?,?)";
            db.query(
              insertFamilleQuery,
              [
                famille.famille,
...
app.get("/total-commandes-par-periode", (req, res) => {
  ...
  const sql = "SELECT COUNT(*) as total FROM commandes WHERE date_commande BETWEEN ? AND ?";
  db.query(sql, [startDate, endDate], (err, result) => {
    if (err) {
      console.error("Error fetching total commandes:", err);
      return res.status(500).send({ error: "An error occurred while fetching the total commandes" });
    }
...
app.get("/etat-de-facturation", (req, res) => {
  ...
  const sql = "SELECT SUM(montant_total_facture) as totalCA FROM facturations WHERE date_facture BETWEEN ? AND ?";
  db.query(sql, [startDate, endDate], (err, result) => {
    if (err) {
      console.error("Error fetching total CA:", err);
      return res.status(500).send({ error: "An error occurred while fetching the total CA" });
    }
