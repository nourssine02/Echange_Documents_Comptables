...
const verifyToken = (req, res, next) => {
  ...
  jwt.verify(token, secretKey, (err, user) => {
    ...
    req.user = user; // Ajoutez les informations de l'utilisateur à la requête
    next();
  });
};
// const verifyToken = (req, res, next) => {
...
app.post("/achats", verifyToken, (req, res) => {
  ...
  db.query(q, values, (err, data) => {
    ...
    // Add notification
    const notificationMessage = `${userId} added an achat`;
    const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
    db.query(notificationQuery, [userId, notificationMessage], (notifErr, notifData) => {
      if (notifErr) {
