...
app.post("/notifications",verifyToken, (req, res) => {
  ...
  // Assurez-vous que userId et message sont présents dans la requête
  if (!userId || !message) {
      return res.status(400).json({ error: "userId et message sont requis" });
  }

  // Insérez le code pour ajouter la notification dans votre base de données
  // Remplacez cette section par votre propre logique pour ajouter la notification

  // Exemple de code fictif pour ajouter une notification
  const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
  db.query(notificationQuery, [req.user.id, message], (err, result) => {
      if (err) {
...
app.post("/achats", verifyToken, (req, res) => {
  ...
  db.query(q, sanitizedValues, (err, data) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: "Erreur lors de l'ajout d'un Achat." });
    }

    // Ajouter une notification
    const notificationMessage = `${req.user.id} a ajouté un achat pour le client ${req.body.clientId}`;
    const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
    db.query(notificationQuery, [req.user.id, notificationMessage], (notifErr, notifData) => {
      if (notifErr) {
        console.error(notifErr);
        return res.status(500).json({ error: "Erreur lors de l'ajout de la notification." });
      }

      return res.status(200).json({ message: "L'Achat et la notification ont été ajoutés avec succès." });
    });
  });
});
