...
function verifyToken(req, res, next) {
  ...
  jwt.verify(token, secretKey, (err, decoded) => {
    ...
    console.log('Token décrypté avec signature vérifiée :', decoded);
    req.user = {
...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [identite], (err, results) => {
    ...
    bcrypt.compare(mot_de_passe, results[0].mot_de_passe, (err, isMatch) => {
      ...
      if (isMatch) {
        const token = jwt.sign({ identite: results[0].identite, role: results[0].role }, secretKey, { expiresIn: '1d' });
        res.json({ auth: true , token : token , res });
