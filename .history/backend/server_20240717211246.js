...
const UpdateTier = ({ isSidebarOpen }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Récupérer les IDs des banques sélectionnées
      const selectedBanqueIds = tier.banques.map((banque) => {
        return banquesOptions.find((option) => option.label === banque.label)?.value;
      });
  
      console.log("Selected banque IDs:", selectedBanqueIds); // Vérifiez les IDs ici
  
      // Mettre à jour avec les IDs récupérés
      await axios.put(`http://localhost:5000/tiers/${id}`, {
        ...tier,
        banques: selectedBanqueIds, // Utiliser les IDs au lieu des noms de banques
      });
