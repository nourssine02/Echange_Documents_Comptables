...
app.get("/comptables", (req, res) => {
  const q = "SELECT `identite`, `code_user` FROM utilisateurs WHERE `position` = 'Comptable'";
  db.query(q, (err, data) => {
    if (err) return res.json(err);
    return res.json(data);
  });
...
app.get("/banques", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "An error occurred while fetching banques." });
    }
    res.json(results);
  });
});
...
app.post("/achats", verifyToken, (req, res) => {
  ...
  // Convertir document_fichier en Buffer si nécessaire
  const documentBuffer = Buffer.from(document_fichier, 'base64');
  ...
  db.query(q, values, (err, data) => {
    ...
    const notificationMessage = `${req.user.identite} a ajouté un nouveau achat`;
...
app.post("/reglements_emis", verifyToken, (req, res) => {
  ...
  const userId = req.user.id;
  if (!userId) {
    return res.status(400).json({ error: 'User ID is required to add reglement' });
  }
  ...
  try {
    ...
    db.beginTransaction((err) => {
      ...
      db.query(insertReglementQuery, [
        ...
        const payementPromises = payements ? payements.map(payement => {
          return new Promise((resolve, reject) => {
        ...
        const piecePromises = pieces ? pieces.map(piece => {
          return new Promise((resolve, reject) => {
            ...
            db.commit((err) => {
              ...
              db.query(getComptableQuery, (comptableErr, comptableData) => {
                if (comptableErr) {
                  ...
                  return res.status(500).json({ error: "Échec de la récupération du comptable", details: comptableErr.message });
                }
                if (comptableData.length === 0) {
                ...
                db.query(notificationQuery, [comptableId, notificationMessage], (notifErr) => {
                  if (notifErr) {
                    ...
                    return res.status(500).json({ error: "Échec de l'ajout de la notification", details: notifErr.message });
                  }
                  return res.status(200).json({
                    message: "Règlement émis ajouté avec succès et notification envoyée",
                  });
                });
              });
            });
...
app.put("/reglements_emis/:id", verifyToken, async (req, res) => {
  const reglementID = req.params.id;
...
app.get('/check-num-piece/:numPiece', async (req, res) => {
  const { numPiece } = req.params;

  try {
    // Exécuter la requête pour vérifier si le numéro de pièce existe
    const [result] = await db.query('SELECT COUNT(*) as count FROM achats WHERE num_piece = ?', [numPiece]);

    if (result && result.length > 0) {
      const count = result[0].count;
      const exists = count > 0; // Vérifier si le nombre est supérieur à 0
      res.json({ exists });
    } else {
    ...
    if (clientId) {
      ...
      db.query(q, [clientId], (err, data) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération des commandes du client spécifié",
          });
        }
        return res.json(data);
      });
      ...
      db.query(q, [], (err, data) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération de toutes les commandes pour les comptables",
          });
        }
        return res.json(data);
      });
    ...
    if (clientId) {
      ...
      db.query(q, [clientId], (err, data) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération des livraisons du client spécifié",
          });
        }
        return res.json(data);
      });
      ...
      db.query(q, [], (err, data) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération de toutes les livraisons pour les comptables",
          });
        }
        return res.json(data);
      });
  ...
  if (req.user.role === "client") {
    ...
    if (clientId) {
      ...
      db.query(q, [], (err, data) => {
        if (err) {
          return res.status(500).json({
            error: "Erreur lors de la récupération de toutes les facturations",
          });
        }
        return res.json(data);
      });
...
app.post("/facture", verifyToken, (req, res) => {
  ...
  if (!userId) {
    return res.status(400).json({ error: 'User ID is required to add facture' });
  }
  ...
  try {
    ...
    return res.status(400).json({ error: 'Invalid JSON format for facture data' });
  }
  ...
  db.beginTransaction((err) => {
    ...
    db.query(insertFactureQuery, [
      facture.date_facture, facture.num_facture, facture.code_tiers, facture.tiers_saisie,
      facture.reference_livraison, facture.montant_HT_facture, facture.FODEC_sur_facture,
      facture.TVA_facture, facture.timbre_facture, facture.autre_montant_facture, facture.montant_total_facture,
      facture.observations, facture.document_fichier, facture.etat_payement, userId
    ], (err, result) => {
      if (err) {
        console.error("Erreur lors de l'insertion du facture:", err);
        return db.rollback(() => res.status(500).json({ message: "Erreur lors de l'insertion du facture." }));
      }
      ...
      db.query(getComptableQuery, (err, data) => {
        if (err) {
          ...
          return db.rollback(() => res.status(500).json({ error: "Erreur lors de la récupération du comptable." }));
        }

        if (data.length === 0) {
          console.error("Aucun comptable trouvé.");
          return db.rollback(() => res.status(404).json({ error: "Aucun comptable trouvé." }));
        }

        const comptableId = data[0].id;
        const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
        db.query(notificationQuery, [comptableId, notificationMessage], (err) => {
          if (err) {
            console.error("Erreur lors de l'insertion de la notification:", err);
            return db.rollback(() => res.status(500).json({ error: "Erreur lors de l'insertion de la notification." }));
          }

          db.commit((err) => {
            if (err) {
              console.error("Erreur lors du commit de la transaction:", err);
              return db.rollback(() => res.status(500).json({ message: "Erreur lors du commit de la transaction." }));
            }
...
app.get("/num_facture", (req, res) => {
  const query = "SELECT DISTINCT id , num_facture FROM facturations WHERE etat_payement = 1";
  db.query(query, (err, results) => {
    if (err) {
      console.error("Erreur lors de l'exécution de la requête SQL:", err);
      res.status(500).json({
...
app.get("/reglements_recus", verifyToken, (req, res) => {
  ...
  if (req.user.role === "client") {
    ...
    db.query(query, [req.user.identite], (err, rows) => {
      if (err) {
        return res.status(500).json({
          error: "Erreur lors de la récupération des règlements reçus du client",
        });
      }
    ...
    if (clientId) {
      query += " AND r.ajoute_par = ?";
      db.query(query, [clientId], (err, rows) => {
        if (err) {
          return res.status(500).json({
            error: "Erreur lors de la récupération des règlements reçus du client spécifié",
          });
        }
        return res.json(rows);
      });
      ...
      db.query(query, [], (err, rows) => {
        if (err) {
          return res.status(500).json({
            error: "Erreur lors de la récupération de tous les règlements reçus",
          });
        }
        return res.json(rows);
      });
...
app.post("/reglements_recus" , verifyToken, (req, res) => {
  ...
  if (!req.user || !req.user.id) {
    console.error("User ID is undefined. Cannot proceed with adding reglement.");
    return res.status(400).json({ error: "User ID is required to add reglement" });
  }
  const userId = req.user.id;
  ...
  db.query(
    "INSERT INTO reglements_recus (code_tiers, tiers_saisie, montant_total_a_regler, observations, ajoute_par ) VALUES (?, ?, ?, ?, ?)",
    [
  ...
  if (req.user.role === "client") {
    ...
    if (clientId) {
      ...
      db.query(q, [], (err, data) => {
        if (err) {
          return res.status(500).json({
            error: "Erreur lors de la récupération de tous les versements",
          });
        }
        return res.json(data);
      });
...
app.post("/versement", verifyToken, (req, res) => {
  ...
  if (!userId) {
    console.error("User ID is undefined. Cannot proceed with adding versement.");
    return res.status(400).json({ error: "User ID is required to add versement" });
  }
  ...
  db.query("INSERT INTO versements_en_banque SET ?", versementData, (err, result) => {
    if (err) {
    ...
    db.query(
      ...
      (err, result) => {
        if (err) {
          ...
          return res.status(500).send("Erreur lors de l'insertion des payements");
        }
        ...
        db.query(getComptableQuery, (comptableErr, comptableData) => {
          ...
          db.query(
            ...
            (notifErr, notifData) => {
              ...
              return res.status(200).json({
                message: "Versement ajouté et notification envoyée avec succès",
              });
            }
          );
        });
