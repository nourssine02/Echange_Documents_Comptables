...
app.put("/tiers/:id", async (req, res) => {
  ...
  try {
    ...
    await db.query(
      'UPDATE tiers SET code_tiers = ?, date_creation = ?, type = ?, autreType = ?, identite = ?, `MF/CIN` = ?, tel = ?, email = ?, adresse = ?, ville = ?, pays = ?, observations = ? WHERE id = ?',
      [code_tiers, date_creation, type, autreType, identite, MF_CIN, tel, email, adresse, ville, pays, observations, tierId]
    );

    // Supprimer les anciennes banques associées
    await db.query('DELETE FROM tiers_banques WHERE tier_id = ?', [tierId]);

    if (Array.isArray(banques)) {
      // Vérifiez que toutes les banques sont valides
      const [validBanks] = await db.query('SELECT id FROM banques WHERE id IN (?)', [banques]);

      // Assurez-vous que `validBanks` est un tableau d'objets
      if (Array.isArray(validBanks)) {
        const validBankIds = validBanks.map(bank => bank.id);

        // Filtrer les banques non valides
        const filteredBanks = banques.filter(bankId => validBankIds.includes(bankId));

        if (filteredBanks.length > 0) {
          // Insérer les nouvelles banques associées
          const insertBanksQuery = 'INSERT INTO tiers_banques (tier_id, banque_id) VALUES ?';
          const values = filteredBanks.map(banqueId => [tierId, banqueId]);
          await db.query(insertBanksQuery, [values]);
        }
      } else {
        throw new Error('Erreur lors de la récupération des banques valides');
      }
    }
    ...
    await db.rollback();
    console.error(error);
    res.status(500).json({ error: 'Erreur lors de la mise à jour du tier et des banques' });
  }
});
