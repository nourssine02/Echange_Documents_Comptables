...
app.get('/achats/:id', async (req, res) => {
  ...
  try {
    const query = 'SELECT * FROM achats WHERE id = ?';
    const [rows] = await db.execute(query, [achatId]);
    ...
    if (rows.length > 0) {
      ...
      if (achat.document_fichier) {
        achat.document_fichier = achat.document_fichier.toString('base64');
      }

      res.json(rows);
    } else {
      res.status(404).send('Achat non trouvé');
    }
  } catch (error) {
