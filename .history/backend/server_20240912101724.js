...
app.post('/achats', upload.single('document_fichier'), verifyToken, (req, res) => {
  try {
    // Vérifie si le fichier a bien été reçu
    if (!req.file) {
      return res.status(400).json({ error: 'Aucun fichier fourni' });
    }

    // Informations de l'achat à partir de req.body
    const {
      date_saisie,
      code_tiers,
      tiers_saisie,
    ...
    // Identifiant de l'utilisateur à partir du token JWT
    const userId = req.user.id;

    // Insertion de l'achat dans la base de données
    const query = `
      INSERT INTO achats (
    ...
    db.query(query, values, (err, result) => {
      ...
      return res.status(200).json({ message: 'Achat ajouté avec succès' });
    });
  } catch (err) {
