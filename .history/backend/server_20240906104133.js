...
// Récupérer les payements associés
const payementResult = await db.query("SELECT * FROM payements WHERE reglement_emis_id = ?", [reglementID]);
const payements = Array.isArray(payementResult) ? payementResult : [];

// Récupérer les pièces associées
const pieceResult = await db.query("SELECT * FROM pieces_a_regler WHERE reglement_emis_id = ?", [reglementID]);
const pieces = Array.isArray(pieceResult) ? pieceResult : [];
