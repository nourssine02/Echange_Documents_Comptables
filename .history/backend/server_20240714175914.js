...
app.post("/tiers", verifyToken, (req, res) => {
  const {
    ...
    pays,
    observations,
    banques // Assumant que banques est une liste d'objets avec une propriété 'value'
  } = req.body;
  ...
  db.query(q, values, (err, result) => {
    ...
    // Ajout des banques associées au tier
    if (banques && banques.length > 0) {
      const addBanques = banques.map(banque => {
        const banqueQuery = "INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)";
        return new Promise((resolve, reject) => {
          db.query(banqueQuery, [tierID, banque.value], (err, result) => {
            if (err) {
      ...
      Promise.all(addBanques)
        .then(() => {
          // Ajout des notifications pour le comptable et le client
          const notificationMessage = `${req.user.identite} a ajouté un Tier`;
      ...
      // Aucune banque à ajouter
      return res.status(200).json({ message: "Le tier a été ajouté avec succès." });
    }
  });
});
  ...
  db.query(q, values, (err, data) => {
    ...
    db.query(getComptableQuery, (comptableErr, comptableData) => {
      ...
      db.query(notificationQuery, [comptableId, notificationMessage], (notifErr, notifData) => {
        ...
        return res.status(200).json({ message: "Achat and notification added successfully" });
      });
    });
  });
});
