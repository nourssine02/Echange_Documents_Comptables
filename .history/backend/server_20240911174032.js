...
app.post("/achats", upload.single('document_fichier'), verifyToken, (req, res) => {
  ...
  // Ensure the file is correctly received
  const document_fichier = req.file ? `/uploads/${req.file.filename}` : null;
  if (!document_fichier) {
  ...
  const values = [
    ...
    montant_total_piece,
    observations,
    document_fichier, // Ensure the file path is stored in the database
    userId,
  ];
