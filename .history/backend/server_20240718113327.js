...
app.put("/tiers/:id", (req, res) => {
  ...
  db.beginTransaction()
    ...
    .then(() => {
      ...
      return db.query('DELETE FROM tiers_banques WHERE tier_id = ?', [tierId]);
    })
    .then(() => {
      // Insert new banks
      if (Array.isArray(banques) && banques.length > 0) {
        let promises = banques.map(banque => {
          return db.query('SELECT id FROM banques WHERE name = ?', [banque.label])
            .then(results => {
              console.log(`Results for bank ${banque.label}:`, results); // Debugging line
              if (Array.isArray(results) && results.length > 0) {
                const banqueId = results[0].id;
                return db.query('INSERT INTO tiers_banques (tier_id, banque_id) VALUES (?, ?)', [tierId, banqueId]);
              } else {
