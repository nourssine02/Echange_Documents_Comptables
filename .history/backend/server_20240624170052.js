...
app.get("/users/:id", async (req, res) => {
  ...
  try {
    const sqlSelect = "SELECT id, code_entreprise, code_user, identite, position, tel, email, mot_de_passe, mot_de_passe_iv, role FROM utilisateurs WHERE id = ?";
    db.query(sqlSelect, [id], async (err, results) => {
      if (err) {
        console.error("Erreur lors de la récupération de l'utilisateur :", err);
        return res.status(500).json({ error: "Erreur du serveur" });
      ...
      const decryptedPassword = decrypt({
        iv: user.mot_de_passe_iv,
        content: user.mot_de_passe
      });

      user.mot_de_passe = decryptedPassword;
