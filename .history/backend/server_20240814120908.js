...
// Function to extract text from the uploaded invoice
const extractTextFromInvoice = (filePath) => {
  return new Promise((resolve, reject) => {
    Tesseract.recognize(
      filePath,
      'fra', // Specify the language as French
      {
...
// Handle file upload and OCR processing in your Node.js server

app.post('/upload', upload.single('document_fichier'), (req, res) => {
  if (!req.file) {
    return res.status(400).send('No file uploaded.');
  }

  // Traitez le fichier avec OCR et renvoyez la réponse
  const filePath = path.join(__dirname, 'uploads', req.file.filename);

  // Utilisez un outil OCR ici, puis renvoyez les résultats
  const extractedText = 'Votre texte extrait ici'; // Remplacer par la logique OCR
