...
app.get('/taux-retenue_source', (req, res) => {
  db.query('SELECT * FROM taux-retenue_source', (err, results) => {
      if (err) {
...
app.post('/taux-retenue_source', (req, res) => {
  const { rate } = req.body;
  db.query('INSERT INTO tax_rates (rate, active) VALUES (?, true)', [rate], (err, results) => {
      if (err) {
          res.status(500).send(err);
      } else {
          res.send(results);
...
app.put('/taux-retenue_source/:id', (req, res) => {
  const { id } = req.params;
  const { active } = req.body;
  db.query('UPDATE tax_rates SET active = ? WHERE id = ?', [active, id], (err, results) => {
      if (err) {
          res.status(500).send(err);
      } else {
          res.send(results);
