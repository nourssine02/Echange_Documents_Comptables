...
app.post("/pointage", async (req, res) => {
  const userId = req.user.id;
  ...
  // Log des données CSV pour débogage
  ...
  data.forEach((row) => {
    ...
    if (!row.code_tiers || !row.identite_tiers || !row.type_paie) {
    ...
    groupedData[userId].push([
      ...
      row.identite_tiers,
      row.type_paie,
      row["nbre_jours/H_travailles"],
      row["nbre_jours/H_supp"],
      row["nbre_jours/H_absence"],
      row["nbre_jours/H_conges_annuel"],
      row["nbre_jours/H_autres_conges"],
      row.supplement_recu,
      row.avance_salaire,
      row.remboursement_prets,
      row.autres_deductions,
      row.observations,
      userId,
    ]);
  });
  ...
  let sql = `INSERT INTO pointage_personnel 
    (code_tiers, identite_tiers, type_paie, \`nbre_jours/H_travailles\`, \`nbre_jours/H_supp\`, \`nbre_jours/H_absence\`, 
     \`nbre_jours/H_conges_annuel\`, \`nbre_jours/H_autres_conges\`, supplement_recu, avance_salaire, remboursement_prets, 
     autres_deductions, observations, ajoute_par) 
    VALUES ?`;
  ...
  db.query(sql, [values], (err, result) => {
    ...
    db.query(getComptableQuery, (comptableErr, comptableData) => {
      ...
      // Envoyer une notification à chaque comptable trouvé
