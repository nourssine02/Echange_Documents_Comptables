  ...
  db.query(q, values, (err, result) => {
    ...
    addBanques
      .then(() => {
        ...
        let recipientRole = req.user.role === 'comptable' ? 'client' : 'comptable';

        const getUserQuery = "SELECT id FROM utilisateurs WHERE role = ?";
        db.query(getUserQuery, [recipientRole], (userErr, userData) => {
          if (userErr) {
            console.error(`Erreur lors de la récupération de l'utilisateur avec le rôle ${recipientRole} :`, userErr);
            return res.status(500).json({ error: `Erreur lors de la récupération de l'utilisateur avec le rôle ${recipientRole}.` });
          }
          if (userData.length === 0) {
            console.error(`Aucun utilisateur trouvé avec le rôle ${recipientRole}`);
            return res.status(404).json({ error: `Aucun utilisateur trouvé avec le rôle ${recipientRole}.` });
          }

          const recipientId = userData[0].id;

          const notificationQuery = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
          db.query(notificationQuery, [recipientId, notificationMessage], (notifErr, notifData) => {
            if (notifErr) {
...
app.put("/tiers/:id", (req, res) => {
  const { id } = req.params;
  const {
    code_tiers, date_creation, type, identite, MF_CIN, tel, email,
    adresse, ville, pays, observations, autreType, banques
  } = req.body;

  if (!banques || !Array.isArray(banques)) {
    return res.status(400).json({ error: 'Les banques doivent être une liste.' });
  }

  const updateTierQuery = `
    UPDATE tiers
    SET code_tiers = ?, date_creation = ?, type = ?, identite = ?, \`MF/CIN\` = ?,
        tel = ?, email = ?, adresse = ?, ville = ?, pays = ?, observations = ?, autreType = ?
    WHERE id = ?
  `;

  const deleteBanquesQuery = `DELETE FROM tiers_banques WHERE tier_id = ?`;

  const insertBanquesQuery = `
    INSERT INTO tiers_banques (tier_id, banque_id)
    VALUES ${banques.map(() => '(?, ?)').join(', ')}
  `;

  // Extraire les banque_ids depuis banques
  const banqueIds = banques.map(b => b.value);
  ...
  db.beginTransaction((err) => {
    ...
    db.query(updateTierQuery, [
      code_tiers, date_creation, type, identite, MF_CIN, tel, email,
      adresse, ville, pays, observations, autreType, id
    ], (err, results) => {
      if (err) {
        console.error('Erreur lors de la mise à jour du tier :', err);
        return db.rollback(() => {
          res.status(500).json({ error: 'Erreur lors de la mise à jour du tier' });
        });
      }

      db.query(deleteBanquesQuery, [id], (err, results) => {
        if (err) {
          console.error('Erreur lors de la suppression des banques existantes :', err);
          return db.rollback(() => {
            res.status(500).json({ error: 'Erreur lors de la mise à jour des banques du tier' });
          });
        }

        // Vérifiez que les banque_ids existent dans la table banques
        const banqueCheckQuery = `
          SELECT id FROM banques WHERE id IN (${banqueIds.join(', ')})
        `;

        db.query(banqueCheckQuery, (err, results) => {
          if (err) {
            console.error('Erreur lors de la vérification des banques :', err);
            return db.rollback(() => {
              res.status(500).json({ error: 'Erreur lors de la mise à jour des banques du tier' });
            });
          }

          // Comparez les ids trouvés avec les ids demandés
          const existingBanqueIds = results.map(row => row.id);
          const missingBanqueIds = banqueIds.filter(id => !existingBanqueIds.includes(id));

          if (missingBanqueIds.length > 0) {
            console.error('Banques manquantes:', missingBanqueIds);
            return db.rollback(() => {
              res.status(400).json({ error: 'Certaines banques sélectionnées n\'existent pas.' });
            });
          }

          db.query(insertBanquesQuery, banqueIds.flatMap(banqueId => [id, banqueId]), (err, results) => {
            if (err) {
              console.error('Erreur lors de l\'insertion des nouvelles banques :', err);
              return db.rollback(() => {
                res.status(500).json({ error: 'Erreur lors de la mise à jour des banques du tier' });
              });
            }
