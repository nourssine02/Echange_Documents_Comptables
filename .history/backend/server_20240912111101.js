...
app.get("/check-num-piece/:numPiece", async (req, res) => {
  ...
  try {
    ...
    if (rows && rows[0].count > 0) {
      // Si le numéro de pièce existe, renvoyer une erreur 400 avec un message
      return res.status(400).json({ error: "Le numéro de pièce existe déjà" });
    } else {
      // Si le numéro de pièce n'existe pas, renvoyer un succès (pas d'erreur)
