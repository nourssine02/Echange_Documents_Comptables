...
app.post("/reglements_emis", verifyToken, (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(insertReglementQuery, [
      ...
      // Promises for inserting payments
      const payementPromises = payements.map((payement) =>
        new Promise((resolve, reject) => {
          ...
          db.query(insertPayementQuery, [
            ...
            if (err) {
              console.error("Erreur lors de l'insertion du payement:", err);
              return reject(err);
            }
            resolve();
          });
      ...
      // Execute all promises and commit the transaction
