...
app.get("/achats/:id", (req, res) => {
  ...
  db.query(q, [achatID], (err, data) => {
    ...
    if (data.length > 0) {
      ...
      // Si document_fichier existe et est binaire
      if (achat.document_fichier) {
        // Convertir en base64 si le fichier est binaire
        achat.document_fichier = Buffer.from(achat.document_fichier).toString('base64');
      }

      return res.json(achat);  // Renvoie le premier objet achat directement
    } else {
