...
app.get('/achats/:clientId?', verifyToken, (req, res) => {
  let query = `
...
app.get('/client/:clientId', verifyToken, (req, res) => {
  ...
  db.query(query, [clientId], (err, data) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: 'Erreur lors de la récupération des détails du client' });
    }
    if (data.length === 0) {
