...
app.post('/banques', async (req, res) => {
  const { bank } = req.body;
  try {
    await db.query('INSERT INTO banques (name) VALUES (?)', [bank]);
    res.status(201).send('Banque ajoutée avec succès.');
  } catch (error) {
    res.status(500).send('Erreur lors de l\'ajout de la banque.');
  }
});

// Exemple d'une route PUT pour modifier l'état actif d'une banque
app.put('/banques/:id', async (req, res) => {
  const { id } = req.params;
  const { active } = req.body; // Recevoir le nouvel état actif
  ...
  try {
    ...
    if (result.affectedRows > 0) {
      res.status(200).json({ message: 'Banque mise à jour', active });
    } else {
      res.status(404).json({ message: 'Banque non trouvée' });
    }
  } catch (error) {
...
const toggleBank = async (id, active) => {
  try {
    const response = await axios.put(`http://localhost:5000/banques/${id}`, {
      active: !active, // Inverser l'état actuel
    });

    if (response.status === 200) {
      // Affiche le retour de l'API pour vérifier
      console.log('Réponse API:', response.data);

      // Mettez à jour directement l'état des banques sans recharger tout
      setBanks((prevBanks) =>
        prevBanks.map((bank) =>
          bank.id === id ? { ...bank, active: !active } : bank
        )
      );
    } else {
...
app.put('/banques/modif/:id', async (req, res) => {
  const { id } = req.params;
  const { bank } = req.body;
  try {
    await db.query('UPDATE banques SET name = ? WHERE id = ?', [bank, id]);
    res.send('Banque modifiée.');
  } catch (error) {
    res.status(500).send('Erreur lors de la modification de la banque.');
  }
});
...
app.get("/check-num-piece/:numPiece", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query("SELECT COUNT(*) as count FROM achats WHERE num_piece = ?", [numPiece]);
    ...
    if (clientId) {
      ...
      db.query(q, [clientId], (err, data) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération des facturations du client spécifié",
          });
        }
        return res.json(data);
      });
  ...
  if (req.user.role === "client") {
    ...
    db.query(query, [req.user.identite], (err, rows) => {
      if (err) {
        return res.status(500).json({
          error:
            "Erreur lors de la récupération des règlements reçus du client",
        });
      }
    ...
    if (clientId) {
      ...
      db.query(query, [clientId], (err, rows) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération des règlements reçus du client spécifié",
          });
        }
        return res.json(rows);
      });
      ...
      db.query(query, [], (err, rows) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération de tous les règlements reçus",
          });
        }
        return res.json(rows);
      });
    ...
    if (clientId) {
      ...
      db.query(q, [clientId], (err, data) => {
        if (err) {
          return res.status(500).json({
            error:
              "Erreur lors de la récupération des versements du client spécifié",
          });
        }
        return res.json(data);
      });
