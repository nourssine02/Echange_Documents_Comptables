...
app.post("/reglements_emis", verifyToken, (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(insertReglementQuery, [
      ...
      reglement.montant_brut || null,
      reglement.base_retenue_source || null,
      reglement.taux_retenue_source || null,
      reglement.montant_retenue_source || null,
      reglement.montant_net || null,
      ...
      if (err) {
        ...
        return db.rollback(() => 
          res.status(500).json({ message: "Erreur lors de l'insertion du règlement émis." })
        );
      }
      ...
      const payementPromises = payements.map(payement =>
        new Promise((resolve, reject) => {
          const insertPayementQuery = `
            ...
            (modalite, num, banque, date_echeance, montant, reglement_emis_id) 
            VALUES (?, ?, ?, ?, ?, ?)`;
      ...
      const piecePromises = pieces.map(piece =>
        new Promise((resolve, reject) => {
          const insertPieceQuery = `
            INSERT INTO pieces_a_regler 
            (num_piece_a_regler, date_piece_a_regler, montant_piece_a_regler, montant_restant, document_fichier, reglement_emis_id) 
            VALUES (?, ?, ?, ?, ?, ?)`;
          ...
          db.commit(err => {
            ...
            db.query(getComptableQuery, (comptableErr, comptableData) => {
              ...
              db.query(notificationQuery, [comptableId, notificationMessage], (notifErr) => {
                ...
                res.status(200).json({ message: "Règlement émis ajouté avec succès et notification envoyée." });
              });
            });
          });
        })
        .catch(err => {
          console.error("Erreur lors de l'insertion des paiements ou des pièces:", err);
          db.rollback(() => 
            res.status(500).json({ message: "Erreur lors de l'insertion des paiements ou des pièces." })
          );
        });
    });
  });
