...
app.get("/commandes-par-periode", (req, res) => {
  ...
  const sql = `
    SELECT c.*, f.famille, f.sous_famille, f.article, f.quantite, f.prix
    FROM commandes c
    INNER JOIN familles f ON c.id = f.commande_id
  ...
  db.query(sql, params, (err, results) => {
    ...
    // Organize the result by grouping the famille details under each commande
    const commandes = results.reduce((acc, row) => {
      const existingCommande = acc.find(c => c.id === row.id);
      if (existingCommande) {
        existingCommande.familles.push({
          famille: row.famille,
        ...
        acc.push({
          ...
          code_tiers: row.code_tiers,
          tiers_saisie: row.tiers_saisie,
          montant_commande: row.montant_commande,
          date_livraison_prevue: row.date_livraison_prevue,
          observations: row.observations,
          document_fichier: row.document_fichier,
          familles: [{
            famille: row.famille,
            sous_famille: row.sous_famille,
            article: row.article,
