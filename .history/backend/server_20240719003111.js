...
app.put("/tiers/:id", async (req, res) => {
  ...
  try {
    ...
    const values = banques.map(banqueId => [tierId, banqueId]);
    if (values.length > 0) {
    ...
    console.error('Erreur lors de la mise à jour du tier ou des banques :', error);
    res.status(500).json({ error: 'Erreur lors de la mise à jour du tier ou des banques' });
  }
});
