...
app.post("/tiers", verifyToken, (req, res) => {
  ...
  const values = [
    ...
    pays || null,  // Assurez-vous de traiter les valeurs null
    observations || null  // Assurez-vous de traiter les valeurs null
  ];
...
app.put("/tiers/:id", (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(
      ...
      [code_tiers, date_creation, type, identite, MF_CIN, tel, email, adresse, ville, pays, observations, autreType, tierId],
      (err, result) => {
        if (err) {
          return db.rollback(() => {
        ...
        db.query('DELETE FROM tiers_banques WHERE tier_id = ?', [tierId], (err, result) => {
          if (err) {
            return db.rollback(() => {
              console.error('Erreur lors de la suppression des banques associées:', err);
              res.status(500).json({ error: 'Erreur lors de la suppression des banques associées' });
            });
          }

          if (Array.isArray(banques) && banques.length > 0) {
            // Vérifier que les banques existent réellement
            db.query('SELECT id FROM banques WHERE id IN (?)', [banques], (err, validBanks) => {
              if (err) {
                return db.rollback(() => {
                  console.error('Erreur lors de la vérification des banques:', err);
                  res.status(500).json({ error: 'Erreur lors de la vérification des banques' });
                });
              }
              ...
              const filteredBanks = banques.filter(banqueId => validBankIds.includes(banqueId));

              if (filteredBanks.length > 0) {
                // Insérer les nouvelles banques associées
                const values = filteredBanks.map(banqueId => [tierId, banqueId]);
                db.query('INSERT INTO tiers_banques (tier_id, banque_id) VALUES ?', [values], (err, result) => {
                  if (err) {
                    return db.rollback(() => {
                      console.error('Erreur lors de l\'insertion des banques associées:', err);
                      res.status(500).json({ error: 'Erreur lors de l\'insertion des banques associées' });
                    });
                  }
                  ...
                  db.commit((err) => {
                    ...
                    res.status(200).json({ message: 'Tier et banques mis à jour avec succès' });
                  });
                });
              } else {
                ...
                db.commit((err) => {
                  ...
                  res.status(200).json({ message: 'Tier mis à jour avec succès, sans banques à ajouter' });
                });
              }
            });
          } else {
