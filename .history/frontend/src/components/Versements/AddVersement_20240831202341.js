...
const AddVersement = ({ isSidebarOpen }) => {
  ...
  const addPayement = () => {
    setPayements((prevPayements) => Array.isArray(prevPayements) ? [
      ...prevPayements,
  ...
  const removePayement = (index) => {
    setPayements((prevPayements) => prevPayements.filter((_, i) => i !== index));
  };
  ...
  <Select
    ...
    onChange={(option) => {
      const selectedTier = codeTiers.find(
        (tier) => tier.id === option?.value
      );

      setPayements((prevState) => ({
        ...prevState,
        tierId: selectedTier?.id || "", // Stocker l'ID du tiers dans tierId
        code_tiers: selectedTier?.code_tiers || "", // Stocker le code du tiers dans code_tiers
      }));
    ...
    value={
      payement.tierId
        ? {
            ...
            label: `${codeTiers.find((tier) => tier.id === payement.tierId)?.code_tiers || ""} - ${codeTiers.find((tier) => tier.id === payement.tierId)?.identite || ""}`,
          }
        : null
    }
  />
