...
const UpdateVersement = ({ isSidebarOpen }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const retrievedPayements = Array.isArray(data.payements) ? data.payements : [];
        setPayements(
  ...
  const handleChangePayement = (e, index) => {
    ...
    if (name === "code_tiers") {
        ...
        if (selectedTier) {
            updatedPayements[index].tierId = selectedTier.id; // Update tierId
            fetchBanques(selectedTier.id); // Fetch associated banks immediately
        } else {
                      ...
                      <div className="col-md-3">
                        <div className="form-group">
                          ...
                          <Select
                            ...
                            onChange={(option) => {
                              const updatedPayements = [...payements];
                              updatedPayements[index].tierId = option?.value || ""; // Update tierId
                              updatedPayements[index].code_tiers =
                                codeTiers.find((tier) => tier.id === option?.value)?.code_tiers || ""; // Update code_tiers
                              setPayements(updatedPayements);
