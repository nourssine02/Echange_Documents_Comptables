...
const UpdateVersement = ({ isSidebarOpen }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(data); // Log entire response to inspect structure
        ...
        const retrievedPayements = Array.isArray(data.payements) ? data.payements : []; // Ensure it's an array
        console.log('Retrieved payements:', retrievedPayements);
  ...
  const fetchBanques = async (tierId) => {
    try {
      ...
      console.log("Banques récupérées:", response.data); // Vérifiez ici
  ...
  const handleChangePayement = (e, index) => {
    ...
    if (name === "code_tiers") {
      ...
      if (selectedTier) {
        updatedPayements.tierId = selectedTier.id; // Assurez-vous de bien récupérer le tierId
