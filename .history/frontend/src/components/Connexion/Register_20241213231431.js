  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.values(inputValidity).every((valid) => valid)) {
      try {
        ...
        console.log("Réponse du serveur:", response.data); // Ajoutez cette ligne pour vérifier la réponse
        alert("Utilisateur ajouté avec succès !");
        setTimeout(() => {
            navigate("/"); // Rediriger après inscription
        }, 2000);
    } catch (error) {
        console.error("Erreur lors de l'inscription:", error); // Ajoutez cette ligne pour voir l'erreur dans la console
