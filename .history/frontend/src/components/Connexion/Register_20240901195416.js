...
const Register = () => {
  ...
  useEffect(() => {
    const fetchComptableCodes = async () => {
      try {
        const res = await axios.get("http://localhost:5000/code_comptables");
                    ...
                    <div className="form-group">
                      ...
                      >
                        ...
                        {comptableCodes.map((comptableCode) => (
                          <option
                            key={comptableCode.code_comptable}
                            value={comptableCode.code_comptable}
                            style={{ color: "black" }}
                          >
                            {comptableCode.code_comptable}
                          </option>
                        ))}
                      </select>
                      {errors.code_comptable && (
