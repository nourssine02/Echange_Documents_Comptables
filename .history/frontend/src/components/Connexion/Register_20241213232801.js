...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(userData).forEach((key) => {
        console.log('Validation pour le champ:', key, 'avec la valeur:', userData[key]); // Affiche la clé et la valeur
        validateField(key, userData[key]); // Appel à la fonction de validation
    });

    // Affiche l'état de inputValidity après validation
    console.log('État de inputValidity après validation :', inputValidity);

    // Vérifier si tous les champs sont valides avant de soumettre
    ...
    try {
        ...
        // Vérification de l'erreur réseau
        if (error.response) {
            // Erreur retournée par le serveur
            alert(`Erreur du serveur : ${error.response.data.message || 'Erreur inconnue'}`);
        } else if (error.request) {
            // Erreur de réseau (pas de réponse du serveur)
            alert("Erreur de connexion au serveur. Vérifiez votre réseau.");
        } else {
            // Autres erreurs
