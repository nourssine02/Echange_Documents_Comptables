...
const FichePaie = ({ isSidebarOpen }) => {
  ...
  const generatePDF = (fiche) => {
    ...
    doc.text(
      `Date de Saisie: ${new Date(fiche.date_de_saisie).toLocaleDateString()}`,
      20,
      30
    );
    doc.text(`Code Tiers: ${fiche.code_tiers}`, 20, 40);
    doc.text(`Identite du Tiers: ${fiche.identite_tiers}`, 20, 50);
    doc.text(`Type de Paie: ${fiche.nbre_jours_travailles2}`, 20, 60);
    doc.text(`Nbres Jours/H Travaillés: ${fiche.nbre_jours_absence1}`, 20, 70);
    doc.text(`Nbres Jours/H Supp: ${fiche.nbre_jours_absence2}`, 20, 80);
    doc.text(`Nbres Jours/H d'Absence: ${fiche.nbre_jours_conges1}`, 20, 90);
    doc.text(`Nbres Jours/H de Congé Annuel: ${fiche.nbre_jours_conges2}`, 20, 100);
    doc.text(`Nbres Jours/H d' Autres Congés: ${fiche.nbre_jours_conges2}`, 20, 100);
    doc.text(`Supplement Reçu: ${fiche.supplement_recus}`, 20, 110);
    doc.text(`Avances sur Salaires: ${fiche.sommes_rejetees}`, 20, 120);
    doc.text(`Remboursement de Prets: ${fiche.remboursement_divers}`, 20, 130);
    doc.text(`Autres Déductions: ${fiche.autre_deduction}`, 20, 140);
