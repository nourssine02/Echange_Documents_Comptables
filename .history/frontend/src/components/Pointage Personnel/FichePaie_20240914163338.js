...
const FichePaie = ({ isSidebarOpen }) => {
  ...
  const generatePDF = (fiche) => {
    ...
    doc.text(`Identité du Tiers: ${fiche.identite_tiers}`, 20, 50);
    doc.text(`Type de Paie: ${fiche.type_paie}`, 20, 60);
    doc.text(`Nbres Jours/H Travaillés: ${fiche.nbre_jours_travailles}`, 20, 70);
    doc.text(`Nbres Jours/H Supp: ${fiche.nbre_jours_supp}`, 20, 80);
    doc.text(`Nbres Jours/H d'Absence: ${fiche.nbre_jours_absence}`, 20, 90);
    doc.text(`Nbres Jours/H de Congé Annuel: ${fiche.nbre_jours_conges_annuel}`, 20, 100);
    doc.text(`Nbres Jours/H d' Autres Congés: ${fiche.nbre_jours_autres_conges}`, 20, 110);
              ...
              <table className="table table-hover">
                <thead>
                  <tr>
                    ...
                    <th>Type de Paie</th>
                    <th>Nombres Jours/H Travaillés</th>
                    <th>Nombres Jours/H Supp</th>
                    <th>Nombres Jours/H d'Absence</th>
                    <th>Nombres Jours/H de Congé Annuel</th>
                    <th>Nombres Jours/H d'Autres Congés</th>
                    <th></th>
                    <th></th>
                  </tr>
                </thead>
                <tbody>
                  {fiches.map((fiche) => (
                    <tr key={fiche.id}>
                      ...
                      <td>{fiche.nbre_jours_travailles}</td>
                      <td>{fiche.nbre_jours_supp}</td>
                      <td>{fiche.nbre_jours_absence}</td>
                      <td>{fiche.nbre_jours_conges_annuel}</td>
                      <td>{fiche.nbre_jours_autres_conges}</td>
                      <td>
                        <button
                          type="button"
