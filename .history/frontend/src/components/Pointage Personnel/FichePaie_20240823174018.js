              ...
              {fiches.map((fiche) => (
                detailsVisible[fiche.id] && (
                  <div key={fiche.id} className="details-section mt-3" style={{display: "flex"}} >                    
                    <div style={{marginRight : "20px"}}><strong>Sommes Rejetées:</strong> {fiche.sommes_rejetees}</div>
                    <div style={{marginRight : "20px"}}><strong>Remboursement Divers:</strong> {fiche.remboursement_divers}</div>
                    <div style={{marginRight : "20px"}}><strong>Autre Déduction:</strong> {fiche.autre_deduction}</div>
                  </div>
                )
              ))}
            </div>
