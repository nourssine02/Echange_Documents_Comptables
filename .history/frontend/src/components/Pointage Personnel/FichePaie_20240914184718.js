...
const FichePaie = ({ isSidebarOpen }) => {
  ...
  const paginatedFiches = fiches.slice(offset, offset + itemsPerPage);
  const pageCount = Math.ceil(fiches.length / itemsPerPage);

  // Fetch data according to user role
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (user.role === "comptable") {
          response = await axios.get(`http://localhost:5000/pointage?code_entreprise=${user.code_entreprise}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
        } else {
          response = await axios.get("http://localhost:5000/pointage", {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
