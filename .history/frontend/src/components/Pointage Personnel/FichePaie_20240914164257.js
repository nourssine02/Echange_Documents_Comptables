...
const FichePaie = ({ isSidebarOpen }) => {
  ...
  const generatePDF = (fiche) => {
    ...
    doc.text(`Date de Saisie: ${new Date(fiche['DATE DE SAISIE']).toLocaleDateString()}`, 20, 30);
    ...
    doc.text(`Nbres Jours/H Travaillés: ${fiche["NBRES DE JOURS OU D'H TRAVAILLES"]}`, 20, 70);
    doc.text(`Nbres Jours/H Supp: ${fiche["NBRES DE JOURS OU D'H SUPP."]}`, 20, 80);
    doc.text(`Nbres Jours/H d'Absence: ${fiche["NBRES DE JOURS OU D'H D'ABSENCE"]}`, 20, 90);
    doc.text(`Nbres Jours/H de Congé Annuel: ${fiche["NBRES DE JOURS OU D'H DE CONGE ANNUEL"]}`, 20, 100);
    doc.text(`Nbres Jours/H d' Autres Congés: ${fiche["NBRES DE JOURS OU D'H AUTRES CONGES"]}`, 20, 110);
    doc.text(`Supplement Reçu: ${fiche['SUPPLEMENT RECU']}`, 20, 120);
    doc.text(`Avances sur Salaires: ${fiche['AVANCES SUR SALAIRES']}`, 20, 130);
    doc.text(`Remboursement de Prets: ${fiche['REMBOURSEMENTS DE PRÊTS']}`, 20, 140);
            ...
            <div className="table-responsive table-sm pt-3">
              <table className="table table-hover">
                <thead>
                  <tr>
                    <th>Code Tiers</th>
                    <th>Identité <br></br>du Tiers</th>
                    <th>Type de <br></br>Paie</th>
                    <th>Nombres <br></br>Jours/H Travaillés</th>
                    <th>Nombres <br></br>Jours/H  Supp</th>
                    <th>Nombres <br></br>Jours/H  d'Absence</th>
                    <th>Nombres <br></br>Jours/H de Congé Annuel</th>
                    <th>Nombres <br></br>Jours/H d'Autres Congés</th>
                    <th></th>
                    <th></th>
                  </tr>
                </thead>
