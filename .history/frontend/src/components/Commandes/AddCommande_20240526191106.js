...
const AddCommande = () => {
  ...
  const handleChangeFamille = async (value, index, field) => {
    const updatedFamilles = [...familles];
    if (field) {
      updatedFamilles[index][field] = value.target ? value.target.value : value.value;
    } else {
      if (value.__isNew__) { // Vérifier si la valeur est nouvelle (saisie au clavier)
        // Ajouter la nouvelle famille à la base de données
        try {
          const res = await axios.post("http://localhost:5000/familles", { famille: value.value });
          ...
          console.log("Error adding new famille:", err);
        }
      } else {
        updatedFamilles[index].famille = value.value;
        // Récupérer les détails de la famille depuis la base de données
        try {
          const res = await axios.get(`http://localhost:5000/familles/${value.value}`);
          const { sous_famille, article, quantite, prix } = res.data;
          updatedFamilles[index] = {
            ...updatedFamilles[index],
            sous_famille,
          ...
          console.log("Error fetching famille data:", err);
        }
      }
    }
    setFamilles(updatedFamilles);
