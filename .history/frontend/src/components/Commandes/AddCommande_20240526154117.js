...
const AddCommande = () => {
  ...
  const handleChangeFamille = async (newValue, index) => {
    const updatedFamilles = [...familles];
    updatedFamilles[index].famille = newValue.value;
    setFamilles(updatedFamilles);

    if (newValue.__isNew__) {
      try {
        await axios.post("http://localhost:5000/familles", { famille: newValue.value });
      } catch (err) {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/familles/${newValue.value}`);
