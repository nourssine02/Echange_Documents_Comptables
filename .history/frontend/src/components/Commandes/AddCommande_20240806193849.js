...
const AddCommande = ({ isSidebarOpen }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "document_fichier" && files.length > 0) {
      ...
      reader.onload = () => {
        ...
        const url = `data:${files[0].type};base64,${base64Data}`;
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case "date_commande":
        error = value ? "" : "La date de la commande est requise";
        break;
      case "num_commande":
        error = value ? "" : "Le numéro de commande est requis";
        break;
      case "code_tiers":
        error = value ? "" : "Le code tiers est requis";
        break;
      case "montant_commande":
        error = value ? "" : "Le montant de la commande est requis";
        break;
      case "date_livraison_prevue":
        error = value ? "" : "La date de livraison prévue est requise";
        break;
      case "document_fichier":
        error = value ? "" : "Document est requis";
        break;
      default:
        break;
    }
  ...
  const validateAllFields = () => {
    ...
    Object.keys(commande).forEach((key) => {
      const error = validateField(key, commande[key]);
      if (error) {
