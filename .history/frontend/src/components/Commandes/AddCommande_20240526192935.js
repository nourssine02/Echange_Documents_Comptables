...
const AddCommande = () => {
  ...
  const handleChangeFamille = async (value, index, field) => {
    ...
    if (field) {
      updatedFamilles[index][field] = value.target
        ? value.target.value
        : value.value;
    } else {
      if (value.__isNew__) {
        // Vérifier si la valeur est nouvelle (saisie au clavier)
        // Ajouter la nouvelle famille à la base de données
        try {
          const res = await axios.post("http://localhost:5000/familles", {
            famille: value.value,
          });
        ...
        try {
          const res = await axios.get(
            `http://localhost:5000/familles/${value.value}`
          );
          const { sous_famille, article, quantite, prix } = res.data;
          updatedFamilles[index] = {
            ...updatedFamilles[index],
