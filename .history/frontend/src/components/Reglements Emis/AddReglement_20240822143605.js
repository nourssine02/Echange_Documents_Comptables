              ...
              <div className="row">
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    <Select
            options={codeTiers.map((tier) => ({ value: tier.id, label: tier.code_tiers }))}
            onChange={(option) => {
              setReglement({ ...reglement, code_tiers: option?.value || "" });
              fetchBanques(option?.value || "");
            }}
            value={codeTiers.find((tier) => tier.id === reglement.code_tiers) ? { value: reglement.code_tiers, label: codeTiers.find((tier) => tier.id === reglement.code_tiers).code_tiers } : null}
          />
                    {/* <select
                      style={{ color: "black" }}
                      className="form-control"
