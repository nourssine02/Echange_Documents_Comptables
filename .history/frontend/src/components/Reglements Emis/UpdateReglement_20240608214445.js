...
const UpdateReglement = ({ isSidebarOpen }) => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [id, initialPayementState, initialPieceState]); // Added missing dependencies
  ...
  return (
    <div className={`app ${isSidebarOpen ? "sidebar-open" : ""}`}>
      <h2>Mettre à jour un règlement émis</h2>
      <form onSubmit={handleSubmit}>
          ...
          <div className="col-md-4">
            <div className="form-group">
              ...
              >
                <option value="">Sélectionnez une option</option>
                {taux.map((tauxItem) => (
                  <option key={tauxItem.id} value={tauxItem.taux}>
                    {tauxItem.taux}
                  </option>
                ))}
              </select>
        ...
        <div className="form-group">
          <button
            type="button"
            className={`btn ${
              activeTab === "payements" ? "btn-primary" : "btn-secondary"
            }`}
          ...
          <button
            type="button"
            className={`btn ${
              activeTab === "pieces" ? "btn-primary" : "btn-secondary"
            }`}
