...
const AddReglement = ({ isSidebarOpen }) => {
    ...
    const fetchPieces = async () => {
      try {
        ...
        const piecesOptions = response.data.map((piece) => {
          const formattedDate = new Date(piece.date_piece).toISOString().split('T')[0]; // Format date to YYYY-MM-DD
          return {
  ...
  const handleSelectChange = (selectedOption, index) => {
    ...
    updatedPieces[index] = {
      ...
      montant_piece_a_regler: selectedOption ? selectedOption.montant_total_piece : "",
  ...
  const handleChange = (e) => {
    ...
    setReglement((prevReglement) => {
      ...
      if (name === 'montant_brut' || name === 'montant_retenue_source' || name === 'base_retenue_source' || name === 'taux_retenue_source') {
  ...
  if (name === "code_tiers") {
    const tierId = value.split(' - ')[0]; // Extracting tier ID
    fetchBanques(tierId);
  }
  };
                  ...
                  <div className="form-group">
                    ...
                    <option value="">Sélectionner le Code Tiers</option>
                      {codeTiers.map((tier) => (
                        <option
                          key={tier.id}
                          value={`${tier.id} - ${tier.code_tiers} - ${tier.identite}`}
                        >                        
