...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (piece) => {
    ...
    console.log("Document piece:", piece);
    console.log("Type of document_fichier:", typeof piece.document_fichier);

    // Check if document_fichier is a Base64 string or URL
    if (typeof piece.document_fichier === "string") {
      if (
        piece.document_fichier.startsWith("data:image") ||
        piece.document_fichier.startsWith("http")
      ) {
        // Valid data URL or image URL
        imageUrl = piece.document_fichier;
      } else {
        console.error(
          "Unsupported document format: String not recognized as URL or Base64."
        );
        return;
      }
    } else if (piece.document_fichier instanceof ArrayBuffer) {
      // Convert ArrayBuffer to Base64 string
      imageUrl = `data:image/jpeg;base64,${arrayBufferToBase64(
        piece.document_fichier
      )}`;
    } else if (piece.document_fichier instanceof Blob) {
      // Convert Blob to Base64 string
      const reader = new FileReader();
      reader.onload = function (event) {
        imageUrl = event.target.result;
        openInNewWindowWithImageUrl(imageUrl);
      };
      reader.onerror = function () {
        console.error("Error reading the Blob.");
      };
      reader.readAsDataURL(piece.document_fichier);
      return;
    } else if (piece.document_fichier instanceof File) {
      ...
      console.error(
        "Unsupported document format: Document type not recognized."
      );
      return;
    }

    // Open the document in a new window if imageUrl is set
    if (imageUrl) {
      openInNewWindowWithImageUrl(imageUrl);
    } else {
      console.error(
        "The document is not available or the format is not supported."
      );
    }
  };
  ...
  // Helper function to open the document in a new window with an image URL
  const openInNewWindowWithImageUrl = (imageUrl) => {
