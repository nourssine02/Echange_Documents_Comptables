...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Valider tous les champs de `achat` avant la soumission
    ...
    if (!hasErrors) {
      ...
      formData.append('achat', JSON.stringify(achat));
      ...
      try {
        ...
        // Naviguer vers la page des achats après un délai
        setTimeout(() => {
          navigate("/achats");
        }, 2000);
      } catch (err) {
      ...
      setAlert({
        type: "danger",
        message: "Veuillez vérifier les champs requis!",
      });
    }
