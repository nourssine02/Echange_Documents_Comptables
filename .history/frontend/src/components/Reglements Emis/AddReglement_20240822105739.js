...
useEffect(() => {
  ...
  const handleChange = (e) => {
    ...
    setReglement((prevReglement) => {
      ...
      if (name === "base_retenue_source" || name === "taux_retenue_source") {
        ...
        // Calcul du montant retenue source
    ...
    // Affichage du modal lorsque tiers_saisie est rempli
  ...
  const handleSubmit = async (e) => {
    ...
    // Vérifier s'il y a des erreurs de validation après la validation des champs
    const hasErrors = Object.values(errors).some((error) => error !== "");
  
    if (!hasErrors) {
      const formData = new FormData();
      
      // Ajouter les détails du règlement
      ...
      try {
        const token = localStorage.getItem("token");
        const axiosInstance = axiosWithAuth(); // Assurez-vous que `axiosWithAuth` est défini
        ...
        // Naviguer vers la page des règlements après un délai
        setTimeout(() => {
          navigate("/reglements_emis");
        }, 2000);
  
      } catch (err) {
        console.error("Erreur lors de l'ajout du règlement :", err);
        Swal.fire({
          icon: "error",
          title: "Erreur",
