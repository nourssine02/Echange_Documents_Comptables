...
const UpdateReglement = ({ isSidebarOpen }) => {
  ...
  <select
    style={{ color: "black" }}
    value={payements[index]?.banque || ""} // S'assurer que la banque pour ce paiement est bien définie
    name="banque"
    className="form-control mr-3"
    onChange={(e) => handleChangePayement(e, index)} // Mettre à jour la banque pour ce paiement spécifique
  >
  ...
  onChange={(option) => {
    setReglement((prevState) => ({
      ...prevState,
      tierId: option?.value || "", // Mettre à jour le tierId
      code_tiers: codeTiers.find((tier) => tier.id === option?.value)
        ?.code_tiers || "", // Stocker le code du tiers
    }));
    fetchBanques(option?.value || ""); // Appeler fetchBanques avec le tierId sélectionné
  }}
  value={
    codeTiers.find((tier) => tier.id === reglement.tierId)
      ? {
          ...
          label: `${
            codeTiers.find((tier) => tier.id === reglement.tierId).code_tiers
          } - ${
            codeTiers.find((tier) => tier.id === reglement.tierId).identite
          }`,
        }
      : null
  }
