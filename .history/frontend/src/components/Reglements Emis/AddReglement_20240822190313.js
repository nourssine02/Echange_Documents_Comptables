...
<Select
  ...
  onChange={(option) => {
    setReglement((prevState) => ({
      ...
      code_tiers: codeTiers.find((tier) => tier.id === option?.value)?.code_tiers || "" // Stocker le code du tiers dans code_tiers
    }));
    fetchBanques(option?.value || "");
  }}
  value={codeTiers.find((tier) => tier.id === reglement.tierId) 
    ? { 
