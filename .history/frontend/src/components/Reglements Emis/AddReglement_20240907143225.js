...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (piece) => {
    ...
    if (typeof piece.document_fichier === "string") {
        if (piece.document_fichier.startsWith("data:image") || piece.document_fichier.startsWith("http")) {
            // Valid data URL or image URL
            imageUrl = piece.document_fichier;
        } else {
            console.error("Unsupported document format: String not recognized as URL or Base64.");
            return;
        }
    } else if (piece.document_fichier instanceof ArrayBuffer) {
        // Convert ArrayBuffer to Base64 string
        imageUrl = `data:image/jpeg;base64,${arrayBufferToBase64(piece.document_fichier)}`;
    ...
    if (imageUrl) {
        ...
        console.error("The document is not available or the format is not supported.");
    }
};
