...
const UpdateReglement = ({ isSidebarOpen }) => {
  ...
  const initialReglementState = {
    date_saisie: new Date().toISOString().split("T")[0], // Date du jour par défaut
    code_tiers: "",
    tiers_saisie: "",
    montant_brut: "",
    base_retenue_source: "",
  ...
  // Fetch des tiers et des taux de retenue
  useEffect(() => {
  ...
  // Fetch des banques liées à un tiers
  ...
  const handleChange = (e) => {
    ...
    if (name === "code_tiers") {
      ...
      if (selectedCodeTier) {
        ...
        fetchBanques(selectedCodeTier.id);
      } else {
        setReglement((prev) => ({
          ...prev,
          code_tiers: "",
  ...
  // Ajout/Suppression de payements et de pièces
  ...
  <div className="form-group">
    ...
    >
      ...
      {banques.map((banque) => (
        <option key={banque.id} value={banque.id}> {/* Utilise l'ID ici */}
          {banque.name}
        </option>
      ))}
    </select>
...
<div className="col-md-3">
  <div className="form-group">
    ...
    <input
      ...
      name="date_echeance"
      onChange={(e) => handleChangePayement(e, index)}
      value={payement.date_echeance || ""} // Valeur par défaut si undefined
    />
  </div>
</div>
