...
console.log(imageUrl);
  ...
  const handleChange = (e) => {
    ...
    setReglement((prevReglement) => {
      ...
      if (name === "code_tiers") {
        ...
        if (selectedTier) {
          updatedReglement.tierId = selectedTier.id;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("payements", JSON.stringify(formattedPayements));
      formData.append("pieces", JSON.stringify(formattedPieces));

      pieces.forEach((piece, index) => {
        if (piece.document_fichier instanceof File) {
          formData.append(`piece_fichier_${index}`, piece.document_fichier);
        }
      });

      await axios.put(`http://localhost:5000/reglements_emis/${id}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      Swal.fire("Success", "Règlement mis à jour avec succès.", "success");
