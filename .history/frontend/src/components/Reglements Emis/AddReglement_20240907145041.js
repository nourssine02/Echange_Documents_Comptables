...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (piece) => {
    ...
    // Check if document_fichier contains a Buffer
    if (piece.document_fichier && piece.document_fichier.type === 'Buffer') {
      // Convert the Buffer data to a Base64-encoded string
      const base64String = Buffer.from(piece.document_fichier.data).toString('base64');
