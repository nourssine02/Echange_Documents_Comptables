...
const ReglementsEmis = ({isSidebarOpen}) => {
  ...
  const filteredReglementsByClient = selectedClient
  ? filteredReglements.filter((reglement) => reglement.identite === selectedClient)
  : filteredReglements;

  const offset = currentPage * itemsPerPage;
  const currentItems = filteredReglementsByClient.slice(
    offset,
    offset + itemsPerPage
  );
  const pageCount = Math.ceil(filteredReglementsByClient.length / itemsPerPage);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete("http://localhost:5000/reglements_emis/" + id);
                  ...
                  <table className="table table-sm">
                    <thead>
                      <tr>
                        ...
                        <th>Montant Brut</th>
                        <th>
                            Taux de la <br></br> retenue à la source
                        </th>
                        <th>
                            Montant Net
                        </th>
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <Link to={`/detailsReglement/${reglement.id}`}>
                              <button type="button" className="btn btn-primary ml-6">
                                Détails
                              </button>
                            </Link>
                            &nbsp;
