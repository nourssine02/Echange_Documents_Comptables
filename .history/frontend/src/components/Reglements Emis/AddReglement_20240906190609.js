...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (piece) => {
    ...
    if (piece.document_fichier) {
      // Check if document_fichier is a Base64 string or File object
      if (typeof piece.document_fichier === "string") {
        const mimeType = piece.document_fichier.split(";")[0].split(":")[1]; // Extract MIME type
        fileUrl = piece.document_fichier.startsWith("data:")
          ? piece.document_fichier
          : `data:${mimeType};base64,${piece.document_fichier}`;
      } else {
        console.error("Le document n'est pas disponible au format Base64.");
        return;
      }
    } else {
    ...
    if (newWindow) {
      if (fileUrl.includes("image")) {
        newWindow.document.write(`
          ...
          <body>
            ...
            <div class="print">
              ...
              <a href="${fileUrl}" download="document_image.jpg">
        ...
        newWindow.document.write(`
          <html>
          <body>
            <p>Le type de fichier n'est pas pris en charge pour l'affichage.</p>
          </body>
          </html>
        `);
      }
  ...
  const handleChangePiece = (e, index) => {
    ...
    if (name === "document_fichier" && files.length > 0) {
      ...
      reader.onload = () => {
        updatedPieces[index][name] = reader.result; // Base64 string
        setPieces(updatedPieces);
      };
                ...
                <div className="row">
                  ...
                  <div className="col-md-3">
                    <div className="form-group">
                      ...
                      {piece.document_fichier && (
                        <img
                        src={`data:image/png;base64,${piece.document_fichier}`}
