...
const UpdateReglement = ({ isSidebarOpen }) => {
  ...
  const handleChangePayement = (e, index) => {
    ...
    updatedPayements[index] = { ...updatedPayements[index], [name]: value };
     ...
     // Si on change la modalité et c'est un paiement lié à une banque, on met à jour les options de banque
     if (name === "modalite" && value === "Chèque") {
      fetchBanques(reglement.tierId);
    }
