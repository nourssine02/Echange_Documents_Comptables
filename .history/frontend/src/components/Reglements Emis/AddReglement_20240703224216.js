...
const AddReglement = ({ isSidebarOpen, user }) => {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case "code_tiers":
        ...
        break;
      case "tiers_saisie":
        error = value ? "" : "Tiers saisi n'est pas obligatoire";
        break;
      case "montant_brut":
  ...
  //     if (!reglement[field]) {
  //       valid = false;
  //       newErrors[field] = `${field.replace(/_/g, " ")} est obligatoire`;
  //     }
  //   });
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(reglement).forEach((key) => validateField(key, entreprise[key]));
