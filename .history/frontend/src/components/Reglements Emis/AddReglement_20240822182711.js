...
const AddReglement = ({ isSidebarOpen }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [tiersResponse, tauxResponse, piecesResponse] = await Promise.all([
          axios.get("http://localhost:5000/code_tiers", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:5000/taux_retenue_source/active", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:5000/pieces", { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case "montant_brut":
        error = value && !isNaN(parseFloat(value)) ? "" : "Le montant brut est obligatoire et doit être un nombre valide";
        break;
      case "base_retenue_source":
        error = value && !isNaN(parseFloat(value)) ? "" : "La base retenue à la source est obligatoire et doit être un nombre valide";
        break;
      case "taux_retenue_source":
        error = value && !isNaN(parseFloat(value)) ? "" : "Le taux retenue à la source est obligatoire et doit être un nombre valide";
        break;
      case "montant_retenue_source":
        error = value && !isNaN(parseFloat(value)) ? "" : "Le montant retenue à la source est obligatoire et doit être un nombre valide";
        break;
      case "montant_net":
        error = value && !isNaN(parseFloat(value)) ? "" : "Le montant net est obligatoire et doit être un nombre valide";
        break;
      default:
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Validate form
      let valid = true;
      for (const [key, value] of Object.entries(reglement)) {
      ...
      payements.forEach((payement, index) => {
        Object.keys(payement).forEach((key) => formData.append(`payements[${index}][${key}]`, payement[key]));
      });
      pieces.forEach((piece, index) => {
        Object.keys(piece).forEach((key) => {
          if (key === 'document_fichier') {
      ...
      const response = await axiosWithAuth().post("/reglements_emis", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      console.log("Valeurs de reglement :", reglement);
      console.log("Valeurs de payements :", payements);
      console.log("Valeurs de pieces :", pieces);


      if (user.role === "client") {
        // Ajouter une notification si l'utilisateur est un client
        ...
        await axiosWithAuth().post("http://localhost:5000/notifications", notificationData);
      }
      Swal.fire({
        icon: 'success',
        title: 'Succès',
        ...
        navigate(`/reglements_emis/${response.data._id}`);
      });
    } catch (error) {
  ...
  <Select
    options={codeTiers.map((tier) => ({ value: tier.id, label: `${tier.code_tiers} - ${tier.identite}` }))}
    onChange={(option) => {
      setReglement({ ...reglement, code_tiers: option?.value || "" });
      fetchBanques(option?.value || "");
    }}
    value={codeTiers.find((tier) => tier.id === reglement.code_tiers) 
      ? { value: reglement.code_tiers, label: `${codeTiers.find((tier) => tier.id === reglement.code_tiers).code_tiers} - ${codeTiers.find((tier) => tier.id === reglement.code_tiers).identite}` }
      : null}
  />
</div>
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    {errors.montant_brut && <small className="text-danger">{errors.montant_brut}</small>}
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    {errors.base_retenue_source && <small className="text-danger">{errors.base_retenue_source}</small>}
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    {errors.taux_retenue_source && <small className="text-danger">{errors.taux_retenue_source}</small>}
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    {errors.montant_retenue_source && <small className="text-danger">{errors.montant_retenue_source}</small>}
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    {errors.montant_net && <small className="text-danger">{errors.montant_net}</small>}
