...
const UpdateReglement = ({ isSidebarOpen }) => {
  ...
  const openImageViewer = (documentUrl) => {
    ...
    if (imageWindow) {
      ...
      img.src = documentUrl;
      img.style.width = "100%"; // Ajustez la largeur à 100% de la fenêtre
      img.style.height = "auto"; // Ajustez la hauteur automatiquement pour conserver le ratio
      img.style.display = "block"; // Supprime les marges
      img.style.margin = "0 auto"; // Centrer l'image horizontalement
  
      imageWindow.document.body.style.margin = "0"; // Supprime les marges du corps de la fenêtre
      imageWindow.document.body.style.padding = "0"; // Supprime les espacements du corps de la fenêtre
      ...
      try {
        ...
        pieces.forEach(piece => {
          if (piece.document_fichier) {
            const blob = new Blob([piece.document_fichier], { type: 'image/jpeg' }); // Changez le type si nécessaire
            newImageUrls[piece.id] = URL.createObjectURL(blob);
          }
        });
  ...
  {piece.document_fichier && (
    <img
      ...
      onClick={() => openImageViewer(piece.document_fichier)}
    />
  )}
</div>
