  ...
  const openDocumentInNewWindow = (piece) => {
    ...
    if (typeof piece.document_fichier === "string") {
        if (piece.document_fichier.startsWith("data:image")) {
            ...
            console.error("Unsupported document format: URL or Base64 not recognized.");
            return;
        }
    } else if (piece.document_fichier instanceof Buffer) {
        // Convert Buffer to Base64 string
        imageUrl = `data:image/jpeg;base64,${BufferToBase64(piece.document_fichier)}`;
    } else if (piece.document_fichier instanceof Blob || piece.document_fichier instanceof File) {
        // Convert Blob or File object to Base64 string
        const reader = new FileReader();
        reader.onload = function(event) {
            imageUrl = event.target.result;
            openInNewWindow(imageUrl);
        };
...
const BufferToBase64 = (buffer) => {
    // Assuming buffer is a Uint8Array or similar
    return btoa(String.fromCharCode(...new Uint8Array(buffer)));
};

// Helper function to open the document in a new window
const openInNewWindow = (imageUrl) => {
