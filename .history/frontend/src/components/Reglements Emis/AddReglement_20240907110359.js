...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (piece) => {
    ...
    // Check if document_fichier is a string (possibly Base64)
    if (typeof piece.document_fichier === "string" && piece.document_fichier.startsWith("data:image")) {
      ...
      // If it's a Blob or File object, convert it to a Base64 string
      const reader = new FileReader();
      reader.onload = function(event) {
        imageUrl = event.target.result;
  
        // Open the new window and display the image
      ...
      // Start reading the file (Blob or File object)
      reader.readAsDataURL(piece.document_fichier);
      return;
    } else {
      console.error("Le document n'est pas disponible ou le format n'est pas supporté.");
      return;
    }
  
    // If the imageUrl is already set, proceed to open the window
    const newWindow = window.open("", "_blank");
    if (newWindow) {
      newWindow.document.write(`
        <!DOCTYPE html>
