...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  useEffect(() => {
    pieces.forEach((piece, index) => {
      if (piece.document_fichier && piece.document_fichier instanceof Blob) {
      ...
      newWindow.document.write(`
        ...
        <body>
          ...
          <div class="print">
            ...
            <a href="${url}" download="document_image.jpg"><button>Download</button></a>
          </div>
        </body>
        </html>
      `);
  ...
  const handleChangePiece = (e, index) => {
    ...
    if (name === "document_fichier" && files.length > 0) {
      ...
      reader.onload = () => {
        updatedPieces[index][name] = files[0]; // Store Blob file
        setPieces(updatedPieces);
      };
  
      reader.readAsArrayBuffer(files[0]); // Read the file as an ArrayBuffer to get a Blob
    } else {
            ...
            {pieces.map((piece, index) => (
              ...
              {imageUrls[index] && (
                <img
                  ...
                  onClick={() => openDocumentInNewWindow(imageUrls[index])}
                />
              )}
            </div>
                  </div>
