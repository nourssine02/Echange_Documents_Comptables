  ...
  const handleSubmit = async (e) => {  
    ...
    try {  
      ...
      Object.keys(reglement).forEach((key) => {  
        formData.append(`reglement[${key}]`, reglement[key]);  
      });  
      ...
      // Ajout des champs spécifiques requis  
      formData.append('reglement[montant_brut]', reglement.montant_brut);  
      formData.append('reglement[base_retenue_source]', reglement.base_retenue_source);  
      formData.append('reglement[taux_retenue_source]', reglement.taux_retenue_source);  
      formData.append('reglement[montant_retenue_source]', reglement.montant_retenue_source);  
      formData.append('reglement[montant_net]', reglement.montant_net);  
  
      // Récupération du token  
      const token = localStorage.getItem("token");  
      const response = await axios.post("http://localhost:5000/reglements_emis", formData, {  
        headers: {   
      ...
      // Notification si l'utilisateur est un client  
      if (user.role === "client") {  
