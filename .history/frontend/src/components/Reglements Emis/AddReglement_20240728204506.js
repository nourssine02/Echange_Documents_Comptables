...
const AddReglement = ({ isSidebarOpen, user }) => {
  ...
  useEffect(() => {
    // Fetch the pieces from the server
    axios.get('/pieces')
      .then(response => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.values(inputValidity).every((valid) => valid)) {
      ...
      try {
        ...
        setAlert({ type: "success", message: "Reglement Emis ajoutées avec succès." });
                ...
                <div key={index} className="row">
                  <div className="col-md-3">
                    <div className="form-group">
                      ...
                      <Select
        options={options}
        onChange={(selectedOption) => handleChangePiece({ target: { name: 'num_piece_a_regler', value: selectedOption.value } }, index)}
        value={options.find(option => option.value === piece.num_piece_a_regler)}
      />
                    </div>
                  </div>
                  <div className="col-md-3">
