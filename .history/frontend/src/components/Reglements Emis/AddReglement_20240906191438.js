...
const AddReglement = ({ isSidebarOpen }) => {
  ...
  useEffect(() => {
    if (pieces.document_fichier) {
      // Create an Object URL from the Blob
      const objectUrl = URL.createObjectURL(piece.document_fichier);
      setImageUrl(objectUrl);

      // Clean up Object URL when component unmounts or when piece changes
      return () => {
  ...
  const openDocumentInNewWindow = (piece) => {
    ...
    if (piece.document_fichier) {
      if (piece.document_fichier instanceof Blob) {
        // Convert Blob to URL
        fileUrl = URL.createObjectURL(piece.document_fichier);
      } else {
        console.error("Le document n'est pas disponible sous forme de Blob.");
        return;
      }
    } else {
