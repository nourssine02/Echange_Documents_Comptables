...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const handleNewTierSubmit = async (e) => {
    e.preventDefault();
    try {
      const banquesIds = await Promise.all(
        newTier.banques.map(async (banque) => {
          const response = await axiosWithAuth().get(`/banques/id?name=${banque.value}`);
      ...
      const response = await axiosWithAuth().post("/tiers", tierData);
      const addedTier = response.data;
      toast.success("Tier ajouté avec succès !");
  ...
  const handleMultiSelectChange = (selectedOptions) => {
    setNewTier((prev) => ({ ...prev, banques: selectedOptions }));
