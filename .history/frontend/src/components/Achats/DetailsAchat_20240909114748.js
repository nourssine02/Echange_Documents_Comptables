...
const DetailsAchat = ({ isSidebarOpen }) => {
  ...
  const getImageType = (base64String) => {
    if (typeof base64String !== 'string') {
      return 'jpeg'; // Default to JPEG if base64String is not a string
    }

    // Check the first characters of the base64 string to identify MIME type
    if (base64String.startsWith('/9j/')) {
      return 'jpeg'; // JPEG indicator
    } else if (base64String.startsWith('iVBORw0KGgo')) {
      return 'png'; // PNG indicator
    } else {
                ...
                <ul className="list-ticked" style={{ fontSize: "16px" }}>
                  {achat ? (
                    <>
                      ...
                      <li>
                        <strong style={{ color: "#118ab2", fontWeight: "bold" }}>Autre Montant sur la Pièce :</strong> {achat.autre_montant_piece || "Non spécifié"} DT
                      </li>
                      <li>
