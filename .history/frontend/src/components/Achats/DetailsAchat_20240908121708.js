...
const DetailsAchat = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (achat) => {
    ...
    // Vérifier si le document est une image ou un PDF
    const mimeType = achat.mime_type || "image/jpeg"; // Type MIME par défaut
  
    // Générer l'URL Base64 complète avec le préfixe MIME
    const base64Url = `data:${mimeType};base64,${achat.document_fichier.includes('base64') 
      ? achat.document_fichier.split(',')[1]  // Si déjà encodé en base64
      : achat.document_fichier}`;  // Sinon, on utilise la donnée directement
  
    // Ouvrir une nouvelle fenêtre et afficher l'image ou le PDF
