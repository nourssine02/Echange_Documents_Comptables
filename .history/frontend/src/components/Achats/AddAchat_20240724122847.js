...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const banquesOptions = [
    { id: 1, label: "Al Baraka Bank", value: "al-baraka-bank" },
    { id: 2, label: "AMEN BANK", value: "amen-bank" },
    { id: 3, label: "ARAB TUNISIAN BANK", value: "arab-tunisian-bank" },
    { id: 4, label: "ATTIJARI BANK", value: "attijari-bank" },
    { id: 5, label: "BANQUE DE TUNISIE", value: "banque-de-tunisie" },
    { id: 6, label: "BANQUE INTERNATIONALE ARABE DE TUNISIE (BIAT)", value: "biat" },
    { id: 7, label: "BH BANK", value: "bh-bank" },
    { id: 8, label: "BANQUE ZITOUNA", value: "banque-zitouna" },
    { id: 9, label: "UNION INTERNATIONALE DE BANQUES", value: "union-internationale-de-banques" },
  ];
  ...
  const handleNewTierSubmit = async (e) => {
    ...
    try {
      ...
      const addedTier = response.data.data; // Accéder à la clé 'data' de la réponse
  
      console.log("Response data:", addedTier); // Afficher la réponse complète du serveur
  
      toast.success("Tier ajouté avec succès !");
  
      // Assurez-vous que les propriétés existent avant de mettre à jour l'état
                    ...
                    <Col md={4}>
                      <Form.Group controlId="banques">
                        ...
                        <Select
                          ...
                          options={banquesOptions.map(banque => ({
                            ...banque,
                            key: banque.id, // Assurez-vous que chaque option a une clé unique
                          }))}
                          className="basic-multi-select"
                          classNamePrefix="select"
                          onChange={handleMultiSelectChange}
                          //value={newTier.banques}
                          value={newTier.banques.map(banque => ({
                            ...banque,
                            key: banque.id, // Ajoutez la clé ici aussi si vous utilisez un composant personnalisé
                          }))}
