...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const [isNumPieceValid, setIsNumPieceValid] = useState(true); // Nouvelle state pour vérifier la validité de num_piece
  ...
  useEffect(() => {
    if (achat.num_piece) {
      const checkNumPiece = async () => {
        try {
          const response = await axios.get(
            `http://localhost:5000/check-num-piece/${achat.num_piece}`
          );
          if (response.data.exists) {
            setIsNumPieceValid(false); // N° de la pièce déjà existant
            setErrors((prevErrors) => ({
              ...prevErrors,
              num_piece: "N° de la Pièce déjà existant",
            }));
          } else {
            setIsNumPieceValid(true); // N° de la pièce est valide
            setErrors((prevErrors) => ({
              ...prevErrors,
              num_piece: "",
            }));
  ...
  }, [achat.num_piece]); // Utiliser useEffect pour détecter les changements de num_piece
  ...
  const handleClick = async (e) => {
    ...
    try {
      ...
      if (response.data.exists) {
        setIsNumPieceValid(false); // Set state variable to indicate that num_piece is invalid
        setErrors((prevErrors) => ({
          ...prevErrors,
          num_piece: "N° de la Pièce déjà existant",
        }));
      } else {
        setIsNumPieceValid(true); // Set state variable to indicate num_piece is valid
      }
    } catch (error) {
      console.error("Erreur lors de la vérification du N° de la Pièce:", error);
      Swal.fire({
        icon: "error",
        title: "Erreur",
        text: "Erreur lors de la vérification du N° de la Pièce.",
      });
    ...
    if (!isNumPieceValid) {
      Swal.fire({
        icon: "error",
        title: "Erreur",
        text: "N° de la Pièce déjà existant. Veuillez en saisir un autre.",
      });
      setIsSubmitting(false);
      return; // Arrêter la soumission si le numéro de la pièce est déjà existant
    }
