    ...
    try {
      ...
      setAchats((prevAchats) =>
        prevAchats.map((achat) =>
          achat.id === id ? { ...achat, statut: newStatut } : achat
        )
      );
    } catch (err) {
      console.log(err);
                      ...
                      {currentItems.map((achat) => (
                        <tr key={achat.id}>
                          ...
                          <td style={{ color: achat.statut === "non réglée" ? "red" : "green" }}>
                            <select
                              value={achat.statut}
                              onChange={(e) => handleStatutChange(achat.id, e.target.value)}
                              className="form-control"
                            >
                              <option value="réglée en espèces">Réglée en espèces</option>
