...
const UpdateAchat = ({ isSidebarOpen }) => {
  ...
  const openImageViewer = (documentBuffer) => {
    // Ouvrir une nouvelle fenêtre
    const imageWindow = window.open("", "_blank");
    const img = document.createElement("img");
  
    // Convertir le buffer en Blob et lire en tant que Data URL
    const blob = new Blob([new Uint8Array(documentBuffer)], { type: 'image/png' });
    const reader = new FileReader();
  ...
  function openDocumentInNewWindow(achat) {
    ...
    if (achat.document_fichier && achat.document_fichier.data) {
      const base64String = arrayBufferToBase64(achat.document_fichier.data);
