...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const handleNewTierSubmit = async (e) => {
    ...
    try {
      ...
      // Check if the actual tier data is nested within the response
      const tierData = addedTier.data; // Adjust this line based on the actual response structure
  
      if (tierData && tierData.code_tiers && tierData.identite) {
        console.log("New code tiers: ", tierData.code_tiers); // Debugging line
        setAchat((prevAchat) => ({
          ...prevAchat,
          tiers_saisie: `${tierData.code_tiers} - ${tierData.identite}`,
        }));
      } else {
        console.error('Added tier does not contain the expected properties:', tierData);
      }
