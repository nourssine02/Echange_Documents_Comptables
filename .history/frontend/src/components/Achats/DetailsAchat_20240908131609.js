...
{achat.mime_type && achat.mime_type.startsWith('image/') ? (
  // Afficher l'image si le fichier est une image (JPEG, PNG, etc.)
  <img 
    src={`data:${achat.mime_type};base64,${achat.document_fichier}`} 
    alt="Document Image" 
    style={{ maxWidth: "100%", maxHeight: "400px", display: "block", margin: "20px 0" }} 
  />
) : (
  // Gérer les fichiers non-images (PDF, etc.)
  <a 
    href={`data:${achat.mime_type || 'application/octet-stream'};base64,${achat.document_fichier}`} 
    download="document"
    target="_blank"
