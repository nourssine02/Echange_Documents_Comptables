...
const Achats = ({ isSidebarOpen }) => {
  ...
  useEffect(() => {
    const fetchAchats = async () => {
      try {
        const res = await axios.get("http://localhost:5000/achats", {
          ...
          params: {
            clientId: selectedClient,  // Passer le client sélectionné en paramètre
          },
        });
        console.log(res.data);  // Vérifiez les données ici
        setAchats(res.data);
      } catch (err) {
        console.log(err);
      }
    ...
    if (selectedClient) fetchAchats();  // Lancer la requête seulement quand un client est sélectionné
  }, [selectedClient]);
