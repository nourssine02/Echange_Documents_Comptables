...
const DetailsAchat = ({ isSidebarOpen }) => {
  ...
  const convertBinaryToBase64 = (binaryData) => {
    const binaryString = window.atob(binaryData); // décoder en base64
    ...
    const blob = new Blob([bytes], { type: 'image/jpeg' }); // ou le type de fichier correct (image/png, image/jpg, etc.)
...
const handleImageDisplay = () => {
  if (achat.document_fichier) {
    const base64Image = convertBinaryToBase64(achat.document_fichier);
  ...
  {imageUrl ? (
    ...
    <button onClick={handleImageDisplay}>Afficher l'image</button>
  )}
</li>
