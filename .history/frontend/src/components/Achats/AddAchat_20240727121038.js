  ...
  useEffect(() => {
    const fetchBanques = async () => {
      try {
        ...
        // Transformez les données pour le composant Select
        const options = response.data.map(banque => ({
          value: banque.id,
          label: banque.name // Assurez-vous que 'nom' est le champ contenant le nom de la banque
        }));
  ...
  >
    {`${tier.code_tiers} - ${tier.identite}`}
            ...
            >
                      ...
                      <Form.Group controlId="banques">
                        ...
                        <Select
                          ...
                          value={banquesOptions.filter(option =>
                            newTier.banques?.some(b => b.value === option.value)
                          )}
              ...
              <Modal.Footer>
                <Button className="btn btn-light" type="button"  onClick={handleModalClose}>
                  Annuler
                </Button>
                <Button
