...
const UpdateAchat = ({ isSidebarOpen }) => {
  ...
  const validateForm = () => {
    ...
    if (!achat.date_saisie)
      newErrors.date_saisie = "Date de saisie est requise.";
    if (!achat.code_tiers) newErrors.code_tiers = "Code tiers est requis.";
    if (!achat.num_piece)
      newErrors.num_piece = "Numéro de la pièce est requis.";
    if (!achat.type_piece)
      newErrors.type_piece = "Type de la pièce est requis.";
    if (!achat.montant_HT_piece)
      newErrors.montant_HT_piece = "Montant HT est requis.";
    ...
    if (!achat.montant_total_piece)
      newErrors.montant_total_piece = "Montant total est requis.";
    if (!achat.document_fichier)
      newErrors.document_fichier = "Document fichier est requis.";
  ...
  const handleClick = async (e) => {
    ...
    try {
      ...
      setAlert({
        type: "danger",
        message: "Une erreur est survenue lors de la mise à jour de l'achat.",
      });
    }
  };
  ...
  const openDocumentInNewWindow = (achat) => {
    let imageUrl;
    if (achat.document_fichier) {
      imageUrl = `data:image/png;base64,${achat.document_fichier}`;
    } else {
      ...
      newWindow.document.write(`
        ...
        <body>
          ...
          <div class="print">
            ...
            <a href="${imageUrl}" download="document_image.png"><button>Download</button></a>
          </div>
        </body>
        </html>
      `);
                ...
                <div className="col-md-4">
                  ...
                  <div className="form-group">
                    ...
                    {achat.document_fichier && (
                      <img
                        src={`data:image/png;base64,${achat.document_fichier}`}
                        alt="Document"
                        style={{
                          width: "100px",
                          height: "auto",
