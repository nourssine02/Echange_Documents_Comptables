...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const handleChange = (e) => {
   ...
   if (name === "document_fichier" && files.length > 0) {
    ...
    reader.onloadend = () => {
      const binaryData = new Buffer.from(reader.result);
      setAchat((prev) => ({ ...prev, document_fichier: binaryData }));
    };
    reader.readAsArrayBuffer(file);
  }  if (name === "document_fichier" && e.target.files.length > 0) {
      const reader = new FileReader();
      reader.onload = () => {
        const binaryData = reader.result;  // Données binaires
        setAchat((prev) => ({ ...prev, document_fichier: binaryData }));
      };
      reader.readAsArrayBuffer(e.target.files[0]);  // Lire le fichier en tant que ArrayBuffer (binaire)
    }
     else {
