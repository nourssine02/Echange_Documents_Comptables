...
const DetailsAchat = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (achat) => {
    ...
    // Ensure that the Base64 string is valid by checking it starts with a proper MIME type and data indicator
    const mimeType = achat.mime_type || 'image/jpeg'; // Default to 'image/jpeg' if mime_type is not defined
    
    // Validate and build the full base64 data URI if necessary
    let imageUrl;
    if (achat.document_fichier.startsWith('data:')) {
      ...
      newWindow.document.write(`
        ...
        <body>
          ...
          <div class="print">
            ...
            <a href="${imageUrl}" download="document_image.${fileExtension}">
