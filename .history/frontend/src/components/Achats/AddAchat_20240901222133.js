...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    setIsSubmitting(true); // Début de la soumission du formulaire
    ...
    if (!hasErrors) {
      // Vérifier l'unicité du `num_piece` avant soumission
      try {
        const response = await axios.get(`http://localhost:5000/check-num-piece/${achat.num_piece}`);
        if (response.data.exists) {
          Swal.fire({
            icon: "error",
            title: "Erreur",
      ...
      try {
        ...
        // Naviguer vers la page des achats après un court délai
        setTimeout(() => {
          navigate("/achats");
        }, 2000);
      } catch (err) {
