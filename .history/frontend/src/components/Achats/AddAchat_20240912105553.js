...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const checkNumPieceExists = async (numPiece) => {
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Erreur",
        text: "Erreur lors de la vérification du numéro de pièce. Veuillez réessayer.",
      });
  ...
  const handleChange = async (e) => {
    ...
    if (name === "document_fichier" && files.length > 0) {
      ...
      // Si le champ modifié est `num_piece`, vérifier s'il existe déjà
      if (name === "num_piece") {
        const numPieceExists = await checkNumPieceExists(value);
        setIsNumPieceValid(!numPieceExists);

        if (numPieceExists) {
          Swal.fire({
            icon: "error",
            title: "Erreur",
            text: "Le numéro de pièce existe déjà dans la base de données.",
          });
        }
      }
    }
              ...
              <div className="row">
                ...
                <div className="col-md-4">
                  ...
                  <div className="form-group">
                    ...
                    {!isNumPieceValid && (
                      <div className="invalid-feedback">
                        Le numéro de pièce existe déjà.
                      </div>
                    )}
                  </div>
                </div>
