...
const DetailsAchat = ({isSidebarOpen}) => {
  ...
  function openDocumentInNewWindow(achat) {
    let imageUrl;
    if (achat.document_fichier.startsWith("data:image")) {
      // If the file is already in base64 format
      imageUrl = achat.document_fichier;
    } else {
      // If it's binary data, create a Blob and generate URL
      const blob = new Blob([achat.document_fichier], { type: 'image/png' });
    ...
    // Release the URL if it was created using Blob
    if (!achat.document_fichier.startsWith("data:image")) {
