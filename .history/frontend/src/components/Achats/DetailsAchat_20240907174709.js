...
const openDocumentInNewWindow = (achat) => {
  ...
  if (achat.document_fichier) {
    // Define mimeType properly: use achat.mime_type or default to image/jpeg
    const mimeType = achat.mime_type || "image/jpeg"; // Make sure mimeType is always defined
  ...
  const fileExtension = mimeType.split('/')[1]; // Extract file extension before using it in the HTML string
