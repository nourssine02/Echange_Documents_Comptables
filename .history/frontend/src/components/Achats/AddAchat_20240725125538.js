...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const handleNewTierSubmit = async (e) => {
    ...
    try {
      ...
      // Assuming the response data is nested within a `tier` property
      const addedTier = response.data.data;
  
      console.log("Added new tier:", addedTier); 


      const codeTiers = response.data.data.code_tiers;
  
      console.log("Code tiers:", addedTier); 
  
      // Check if the added tier contains the expected properties
      if (addedTier && addedTier.code_tiers && addedTier.identite) {
        console.log("New code tiers: ", addedTier.code_tiers); 
