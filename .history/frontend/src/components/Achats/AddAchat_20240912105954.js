...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  // Nouvelle fonction pour vérifier si le numéro de pièce existe dans la base de données
  const checkNumPieceExists = async (numPiece) => {
    try {
      ...
      let errorMessage = "Erreur lors de la vérification du numéro de pièce. Veuillez réessayer.";
  
      // Gestion des erreurs spécifiques en fonction du code de statut HTTP
      if (error.response) {
        if (error.response.status === 404) {
          errorMessage = "Le numéro de pièce n'existe pas dans la base de données.";
      ...
      return false; // Retourner false par défaut si une erreur se produit
    }
  };
  ...
  const handleChange = async (e) => {
    ...
    if (name === "num_piece") {
      ...
      if (numPieceExists) {
        // Si le numéro de pièce existe, affichez une alerte et marquez le champ comme invalide
        Swal.fire({
          icon: "error",
