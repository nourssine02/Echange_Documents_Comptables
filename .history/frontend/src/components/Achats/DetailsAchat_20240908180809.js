...
const DetailsAchat = ({ isSidebarOpen }) => {
  ...
  }, [id]); // Depend on the id to avoid re-fetching on every render
  ...
  const handleImageDisplay = () => {
    ...
    try {
      ...
      // Update the state with the data URL for display, but do it only if the URL has not been set yet
      if (!achat.dataUrl) {
        setAchat((prevAchat) => ({ ...prevAchat, dataUrl }));
      }
    } catch (error) {
  ...
  useEffect(() => {
    // Call handleImageDisplay only when document_fichier or mime_type changes
    if (achat.document_fichier && !achat.dataUrl) {
      handleImageDisplay();
    }
  }, [achat.document_fichier, achat.mime_type, achat.dataUrl]); // Ensure we don't loop unnecessarily
