...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  // Fonction pour vérifier l'unicité du champ num_piece
  ...
  const handleChange = async (e) => {
    ...
    if (name === "document_fichier" && files.length > 0) {
      // If a file is selected, create a FormData object and append all necessary fields.
      const formData = new FormData();
  
      // Append all the fields from the 'achat' state to FormData, except the file field
      Object.keys(achat).forEach((key) => {
        formData.append(key, achat[key]);
      });
  
      // Append the file to the FormData object
      formData.append("document_fichier", files[0]); // Ensure the file is appended correctly
  
      // Send the FormData object to the backend
      try {
        const axiosInstance = axiosWithAuth();
  
        const response = await axiosInstance.post("http://localhost:5000/achats", formData, {
          headers: {
        ...
        console.log("Achat added successfully:", response.data);
      } catch (error) {
        // Handle any errors during file upload
        console.error("Error uploading file:", error);
      }
    } else {
      // Handle other input field changes (non-file fields)
      setAchat((prev) => ({ ...prev, [name]: value }));
  
      // Validate the field if necessary (assuming you have a `validateField` function)
      validateField(name, value);
  
      // If the field is 'num_piece', debounce the uniqueness check
      if (name === "num_piece") {
        clearTimeout(timeoutRef.current); // Clear any previous timeout
        timeoutRef.current = setTimeout(() => {
          checkNumPieceUniqueness(value); // Check if 'num_piece' is unique after delay
        }, 500); // 500ms debounce time
      }
  
      // If the field is 'tiers_saisie' and it's empty, trigger the modal
      if (name === "tiers_saisie" && value === "") {
        setShowModal(true); // Show modal if tiers_saisie is empty
      }
    }
  };
  ...
  const handleClick = async (e) => {
    ...
    try {
      await axiosInstance.post("http://localhost:5000/achats", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
