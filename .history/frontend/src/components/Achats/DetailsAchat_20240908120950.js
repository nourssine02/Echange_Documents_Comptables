...
const DetailsAchat = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (achat) => {
    ...
    // Vérifier si le fichier Base64 contient le préfixe `data:` et nettoyer si nécessaire
    ...
    const mimeType = achat.mime_type || 'application/octet-stream'; // Type MIME par défaut
  
    // Fonction pour convertir la base64 en Blob
    ...
    // Ouvrir une nouvelle fenêtre et injecter l'image ou PDF selon le type MIME
    const newWindow = window.open("", "_blank");
  
    if (newWindow) {
      // Attendre que la nouvelle fenêtre soit prête avant de manipuler le document
      newWindow.onload = () => {
        const fileExtension = mimeType.split('/')[1] || 'bin'; // Extension de fichier par défaut
  
        if (mimeType.startsWith('image/')) {
          // Si le fichier est une image, l'afficher dans un élément <img>
          ...
          // Si le fichier est un PDF, l'afficher dans un <embed>
          newWindow.document.write(`
            <!DOCTYPE html>
            <html lang="en">
            <head>
          ...
          // Si le type est inconnu, permettre le téléchargement du fichier
          newWindow.document.write(`
            <!DOCTYPE html>
            <html lang="en">
            <head>
            ...
            <body>
              <p>Le fichier ne peut pas être prévisualisé. Cliquez ci-dessous pour le télécharger :</p>
              <a href="${blobUrl}" download="document.${fileExtension}">
