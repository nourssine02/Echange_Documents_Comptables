...
const DetailsAchat = ({ isSidebarOpen }) => {
  ...
  const openDocumentInNewWindow = (achat) => {
    ...
    // Ensure mimeType is defined within the same block and accessible
    const mimeType = achat.mime_type || "image/jpeg";  // Declare mimeType here
    
    if (achat.document_fichier) {
      // Construct the data URL using the mimeType
    ...
    // Extract file extension based on mimeType safely
