...
const AddAchat = ({ isSidebarOpen }) => {
  ...
  const handleNewTierSubmit = async (e) => {
    ...
    try {
      ...
      const addedTier = response.data.newTier;
  
      console.log("Added new tier:", addedTier); // Log the added tier to see its structure
  
      // Check if the added tier contains the expected properties
      if (addedTier && addedTier.code_tiers && addedTier.identite) {
        console.log("New code tiers: ", addedTier.code_tiers); // Debugging line
        setAchat((prevAchat) => ({
          ...prevAchat,
