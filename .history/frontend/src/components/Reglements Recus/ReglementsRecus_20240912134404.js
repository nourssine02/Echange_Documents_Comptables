...
const ReglementsRecus = ({isSidebarOpen}) => {
  ...
  const filteredReglementsRecusByClient = selectedClient
  ? filteredReglements.filter((reglement) => reglement.identite === selectedClient)
  : filteredReglements;

  const offset = currentPage * itemsPerPage;
  const currentItems = filteredReglementsRecusByClient.slice(
    offset,
    offset + itemsPerPage
  );
  const pageCount = Math.ceil(filteredReglementsRecusByClient.length / itemsPerPage);
