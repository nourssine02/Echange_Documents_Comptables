...
const DetailsLivraison = ({isSidebarOpen}) => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (err.response && err.response.status === 500) {
        ...
        toast.error("Impossible de supprimer cette livraison. Elle est liée à d'autres données.");
      } else {
  ...
  const confirmDelete = (id) => {
    const confirmDelete = window.confirm("Voulez-vous vraiment supprimer cette livraison ?");
    if (confirmDelete) {
      handleDelete(id);
    }
  };
          ...
          <div className="row justify-content-center">
            <div className="col-md-8">
              <div className="card">
                ...
                <h3 className="title text-center">Détails de la Livraison</h3>
                <br />
              <ul className="list-arrow" style={{ fontSize: "15px" }}>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >Date du Bon de Livraison:</strong>
                  {new Date(livraison.date_BL).toLocaleDateString()}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }}>N° du Bon de Livraison:</strong> {livraison.num_BL}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >Code Tiers:</strong> {livraison.code_tiers}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >Tiers à Saisir:</strong> {livraison.tiers_saisie}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >Référence Commande:</strong>{" "}
                  {livraison.reference_commande}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >Montant HT du Bon de Livraison:</strong>{" "}
                  {livraison.montant_HT_BL}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >TVA du Bon de Livraison:</strong> {livraison.TVA_BL}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >Montant Total du Bon de Livraison:</strong>{" "}
                  {livraison.montant_total_BL}
                </li>
                <li>
                  <strong style={{ color: "#118ab2", fontWeight: "bold" }} >Observations:</strong> {livraison.observations}
                </li>{" "}
                <li>
