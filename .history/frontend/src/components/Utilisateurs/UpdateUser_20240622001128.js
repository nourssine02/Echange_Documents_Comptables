...
const UpdateUser = ({ isSidebarOpen }) => {
  ...
  const handleClick = async (e) => {
    ...
    const encryptedPassword = CryptoJS.AES.encrypt(user.mot_de_passe, "MY_SECRET_KEY_WAS_ABCD1234").toString();
    axios
      .put(`http://localhost:5000/users/${id}`, { ...user, mot_de_passe: encryptedPassword })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        try {
          const bytes = CryptoJS.AES.decrypt(data.mot_de_passe, "MY_SECRET_KEY_WAS_ABCD1234");
          const decryptedPassword = bytes.toString(CryptoJS.enc.Utf8);
          if (!decryptedPassword) {
            throw new Error("Le mot de passe déchiffré est vide");
          }
          ...
          setUser({
            ...
            tel: data.tel,
            email: data.email,
            mot_de_passe: "",  // Reset mot_de_passe to an empty string in case of error
            role: data.role,
          });
        }
      })
