...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateAllFields()) {
      const formData = new FormData();
      formData.append('date_facture', facture.date_facture || '');
      formData.append('num_facture', facture.num_facture || '');
      formData.append('code_tiers', facture.code_tiers || '');
      formData.append('tiers_saisie', facture.tiers_saisie || '');
      formData.append('reference_livraison', facture.reference_livraison || '');
      formData.append('montant_HT_facture', facture.montant_HT_facture || '');
      formData.append('FODEC_sur_facture', facture.FODEC_sur_facture || '');
      formData.append('TVA_facture', facture.TVA_facture || '');
      formData.append('timbre_facture', facture.timbre_facture || '');
      formData.append('autre_montant_facture', facture.autre_montant_facture || '');
      formData.append('montant_total_facture', facture.montant_total_facture || '');
      formData.append('observations', facture.observations || '');
      formData.append('etat_payement', facture.etat_payement ? 'true' : 'false'); // Convertir le booléen en chaîne
      formData.append('document_fichier', file || ''); // Assurez-vous que `file` est bien défini
