...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  const handleOCR = async (formData) => {
    try {
      ...
      setFacture({
        ...
        num_facture: data.invoiceNumber || "",
        montant_HT_facture: data.montant_HT || "",
        FODEC_sur_facture: data.FODEC || "",
        TVA_facture: data.TVA || "",
        timbre_facture: data.timbre || "",
        montant_total_facture: data.montant_total || "",
      });
      Swal.fire({
        icon: "success",
        title: "Succès",
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      // Insérer les données extraites dans le formulaire
      setFacture({
        dateFacture: response.data.dateFacture || '',
        numFacture: response.data.numFacture || '',
        montantHT: response.data.montantHT || '',
        fodec: response.data.fodec || '',
        tva: response.data.tva || '',
        timbre: response.data.timbre || '',
  ...
  //   } else if (name === "document_fichier" && files.length > 0) {
  //     const file = files[0];
  //     const formData = new FormData();
  //     formData.append('file', file);
  
  //     // Appeler l'API OCR pour analyser le fichier
  //     handleOCR(formData);
  //   } else {
