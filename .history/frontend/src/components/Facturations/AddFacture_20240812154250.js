...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  const extractInfoWithRegex = (extractedText) => {
    ...
    if (dateMatch) {
      date_facture = convertDateToISO(dateMatch[1]);
      console.log("Date trouvée :", date_facture);
    }
    if (numFactureMatch) {
      num_facture = numFactureMatch[1];
      console.log("Numéro de facture trouvé :", num_facture);
    }
    if (montantHTMatch) {
      montant_HT_facture = montantHTMatch[1];
      console.log("Montant HT trouvé :", montant_HT_facture);
    }
    if (FODECMatch) {
      FODEC_sur_facture = FODECMatch[1];
      console.log("FODEC trouvé :", FODEC_sur_facture);
    }
    if (TVAMatch) {
      TVA_facture = TVAMatch[1];
      console.log("TVA trouvée :", TVA_facture);
    }
    if (timbreMatch) {
      timbre_facture = timbreMatch[1];
      console.log("Timbre trouvé :", timbre_facture);
    }
    if (montantTotalMatch) {
      montant_total_facture = montantTotalMatch[1];
      console.log("Montant total trouvé :", montant_total_facture);
    }
  ...
  const handleOCR = async (formData) => {
    try {
      ...
      if (extractedText) {
        ...
        // Utilisation de la fonction pour extraire les informations avec RegEx
        const extractedInfo = extractInfoWithRegex(extractedText);
