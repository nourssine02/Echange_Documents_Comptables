...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  // Fonction pour appeler l'API d'IA pour extraire les informations
  const fetchAIData = async (text) => {
    try {
      // Remplacez par la clé API sécurisée et l'URL correcte
  ...
  const structureInfoWithAI = async (extractedText) => {
    const aiData = await fetchAIData(extractedText);
    if (aiData) {
      setFacture((prev) => ({
        ...prev,
        date_facture: aiData.date_facture || "",
        num_facture: aiData.num_facture || "",
        montant_HT_facture: aiData.montant_HT_facture || "",
        FODEC_sur_facture: aiData.FODEC_sur_facture || "",
        TVA_facture: aiData.TVA_facture || "",
        timbre_facture: aiData.timbre_facture || "",
        montant_total_facture: aiData.montant_total_facture || "",
      }));
    } else {
      Swal.fire({
        icon: "warning",
