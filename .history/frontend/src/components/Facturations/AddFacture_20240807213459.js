  ...
  const handleOCR = async (formData) => {
    try {
      ...
      setFacture((prev) => ({
        ...
        montant_HT_facture: data.montant_HT || prev.montant_HT_facture,
        FODEC_sur_facture: data.FODEC || prev.FODEC_sur_facture,
        TVA_facture: data.TVA || prev.TVA_facture,
        timbre_facture: data.timbre || prev.timbre_facture,
        montant_total_facture: data.montant_total || prev.montant_total_facture,
      }));
      Swal.fire({
        icon: "success",
        title: "Succès",
