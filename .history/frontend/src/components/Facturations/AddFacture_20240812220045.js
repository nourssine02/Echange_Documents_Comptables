  ...
  const structureInfoWithNER = async (extractedText) => {
    ...
    if (nerData && nerData.entities) {
      ...
      nerData.entities.forEach((entity) => {
        ...
        if (entity.label === "DATE" && /\d{2}\/\d{2}\/\d{4}/.test(word)) {
          date_facture = word;
        } else if (entity.label === "MISC" && /facture|Facture/i.test(word)) {
          num_facture = word.match(/\d+/) ? word.match(/\d+/)[0] : num_facture;
        } else if (entity.label === "MONEY") {
          const montant = word.replace(/[^0-9,.]/g, '');
          if (!montant_HT_facture) montant_HT_facture = montant;
          else if (!FODEC_sur_facture) FODEC_sur_facture = montant;
          else if (!TVA_facture) TVA_facture = montant;
          else if (!timbre_facture) timbre_facture = montant;
          else if (!montant_total_facture) montant_total_facture = montant;
        }
      });
