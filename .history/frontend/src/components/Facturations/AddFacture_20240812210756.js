...
const fetchEntities = async (text) => {
  try {
      const response = await axios.post(
          ...
          {
              headers: {
                  Authorization: `Bearer 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
`,
              },
          }
      );
      console.log("Réponse de l'API NLP:", response.data); // Ajoutez ce log pour voir la structure exacte
      return response.data;
  } catch (error) {
      console.error("Erreur lors de la récupération des entités NLP :", error);
      return null;
  }
};
