...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  "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";
  ...
  const handleOCR = async (formData) => {
    try {
      ...
      // setFacture((prev) => ({
      //   ...prev,
      //   date_facture: data.date_facture || prev.date_facture || "",
      //   num_facture: data.num_facture || prev.num_facture || "",
      //   montant_HT_facture: data.montant_HT_facture || prev.montant_HT_facture || "",
      //   FODEC_sur_facture: data.FODEC_sur_facture || prev.FODEC_sur_facture || "",
      //   TVA_facture: data.TVA_facture || prev.TVA_facture || "",
      //   timbre_facture: data.timbre_facture || prev.timbre_facture || "",
      //   montant_total_facture: data.montant_total_facture || prev.montant_total_facture || "",
      // }));
  ...
  const fetchAICompletion = async (prompt) => {
    try {
      const response = await axios.post(
        "https://api.openai.com/v1/completions",
        {
          model: "text-davinci-003",  // Vous pouvez utiliser un modèle gratuit
          prompt: prompt,
          max_tokens: 150,
        },
        {
          headers: {
            ...
            "Authorization": `Bearer sk-nq1QuMFClpY2yFg3de6HPto2gbhnzdLGXXnXjdK9GYT3BlbkFJ41AVYhuCM0T7-VYMRcFKrvO_AnYzvydsjdRcSbn6QA`, // Remplacez par votre clé API
          },
        }
      );
  ...
  const structureInfoWithAI = async (extractedText) => {
    ...
    // Analyser la réponse et mettre à jour les champs
    // Vous devrez ajuster ce code pour extraire les informations du texte renvoyé par l'IA
    // Exemple simple ci-dessous, vous pouvez utiliser regex ou une autre méthode de parsing

    const date_facture = structuredData.match(/Date de la facture : (.*)/)?.[1] || "";
    const num_facture = structuredData.match(/Numéro de la facture : (.*)/)?.[1] || "";
    const montant_HT_facture = structuredData.match(/Montant HT : (.*)/)?.[1] || "";
    const FODEC_sur_facture = structuredData.match(/FODEC : (.*)/)?.[1] || "";
    const TVA_facture = structuredData.match(/TVA : (.*)/)?.[1] || "";
    const timbre_facture = structuredData.match(/Timbre fiscal : (.*)/)?.[1] || "";
    const montant_total_facture = structuredData.match(/Montant total : (.*)/)?.[1] || "";
