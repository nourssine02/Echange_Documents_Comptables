...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  //           setFacture((prev) => ({
  //             ...prev,
  //             date_facture: fields.InvoiceDate?.valueDate || prev.date_facture,
  //             num_facture: fields.InvoiceId?.valueString || prev.num_facture,
  //             montant_HT_facture: fields.Subtotal?.valueNumber || prev.montant_HT_facture,
  //             TVA_facture: fields.Tax?.valueNumber || prev.TVA_facture,
  //             FODEC_sur_facture: fields.Fodec?.valueNumber || prev.FODEC_sur_facture,
  //             timbre_facture: fields.Timbre?.valueNumber || prev.timbre_facture,
  //             montant_total_facture: fields.Total?.valueNumber || prev.montant_total_facture,
  //           }));
            
  //         Swal.fire(
  //           "Succès",
  //           "Les données de la facture ont été extraites!",
  //           "success"
  //         );
  //         break;
  //       }
  ...
  const analyzeInvoiceWithAzure = async (file) => {
    const formData = new FormData();
    formData.append("file", file);

    try {
      const response = await axios.post(
        "https://https://form-recognizer-service.cognitiveservices.azure.com/formrecognizer/v2.1/prebuilt/invoice/analyze?includeTextDetails=true", // Remplacez par votre endpoint Azure
        formData,
        {
          headers: {
            "Ocp-Apim-Subscription-Key": "53fa9f341277462ab1327e5037165f93", // Remplacez par votre clé API
            "Content-Type": "multipart/form-data",
          },
        }
      );
      ...
      console.log("Résultat Azure Form Recognizer:", result);

      const aiData = {};
      const fields = result.analyzeResult.documentResults[0].fields;

      // Extraction des informations pertinentes
      aiData.date_facture = fields.InvoiceDate?.text || "";
      aiData.num_facture = fields.InvoiceId?.text || "";
      aiData.montant_HT_facture = fields.SubTotal?.text || "";
      aiData.TVA_facture = fields.Tax?.text || "";
      aiData.montant_total_facture = fields.Total?.text || "";
      ...
      Swal.fire({
        icon: "success",
        title: "Succès",
        text: "Les informations de la facture ont été extraites avec succès!",
      });
    } catch (error) {
      console.error("Erreur lors de l'utilisation d'Azure Form Recognizer:", error);
      Swal.fire({
        icon: "error",
        title: "Erreur",
        text: "Impossible d'extraire des informations de la facture.",
      });
    }
  };
