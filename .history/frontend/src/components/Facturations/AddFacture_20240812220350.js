...
const AddFacture = ({ isSidebarOpen }) => {
 ...
 const fetchChatGPT = async (text) => {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      const response = await axios.post(
        ...
        {
          model: "gpt-3.5-turbo", // Modèle mis à jour
          prompt: `Extract the following details from the invoice text: ${text}. Provide the date, invoice number, amount HT, FODEC, VAT, stamp, and total amount.`,
          max_tokens: 150,
        },
        {
          headers: {
            Authorization: `Bearer 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`, // Remplacez par votre clé API
  ...
  const structureInfoWithChatGPT = async (extractedText) => {
    ...
    if (chatGPTData) {
      // Vous devrez probablement parser chatGPTData pour extraire les informations spécifiques
      // Cela dépend de la manière dont les données sont formatées dans la réponse de ChatGPT
      ...
      lines.forEach(line => {
        if (line.startsWith("Date:")) date_facture = line.replace("Date:", "").trim();
        if (line.startsWith("Invoice Number:")) num_facture = line.replace("Invoice Number:", "").trim();
        if (line.startsWith("Amount HT:")) montant_HT_facture = line.replace("Amount HT:", "").trim();
        if (line.startsWith("FODEC:")) FODEC_sur_facture = line.replace("FODEC:", "").trim();
        if (line.startsWith("VAT:")) TVA_facture = line.replace("VAT:", "").trim();
        if (line.startsWith("Stamp:")) timbre_facture = line.replace("Stamp:", "").trim();
        if (line.startsWith("Total Amount:")) montant_total_facture = line.replace("Total Amount:", "").trim();
      });
