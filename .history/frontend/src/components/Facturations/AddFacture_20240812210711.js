...
// Fonction pour appeler l'API Hugging Face avec un modèle NLP adapté
const fetchEntities = async (text) => {
  const response = await axios.post(
      "https://api-inference.huggingface.co/models/dbmdz/bert-large-cased-finetuned-conll03-english", // Modèle NLP adapté
      { inputs: text },
      {
          headers: {
...
// Fonction pour structurer les informations extraites avec NLP
const structureInfoWithNLP = async (extractedText) => {
  try {
      const nlpData = await fetchEntities(extractedText);

      if (!nlpData || !nlpData.entities) {
          console.error("Les entités NLP ne sont pas disponibles.");
          return {};
      }
      ...
      // Boucle sur les entités extraites par le modèle NLP
      nlpData.entities.forEach((entity) => {
          if (entity.label === "DATE") {
              date_facture = entity.word;
          } else if (entity.label === "MISC" && entity.word.match(/facture|Facture/)) {
              ...
              else if (!montant_total_facture) montant_total_facture = entity.word;
          }
      });
...
const handleOCR = async (formData) => {
  try {
      ...
      if (extractedText) {
          // Utiliser l'IA pour structurer les informations
          const extractedInfo = await structureInfoWithNLP(extractedText);
