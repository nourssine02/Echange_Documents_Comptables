...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  // Fonction pour structurer les informations extraites via l'IA
  const structureInfoWithAI = async (extractedText) => {
    try {
      const response = await axios.post(
        ...
        {
          model: "gpt-3.5-turbo",
          prompt: `Voici un texte de facture extrait par OCR: "${extractedText}". Structurez ces informations sous forme de JSON avec les champs suivants : date_facture, num_facture, montant_HT_facture, FODEC_sur_facture, TVA_facture, timbre_facture, montant_total_facture.`,
          max_tokens: 150,
          temperature: 0.2,
        },
        {
          headers: {
            Authorization: `Bearer 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`,
            "Content-Type": "application/json",
          },
        }
      );

      const aiData = JSON.parse(response.data.choices[0].text.trim());
      ...
      if (aiData) {
        ...
        throw new Error("Aucune information extraite");
      }
    } catch (error) {
