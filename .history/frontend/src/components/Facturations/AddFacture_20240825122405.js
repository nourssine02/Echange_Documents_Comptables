...
const AddFacture = ({ isSidebarOpen }) => {
    ...
    try {
      ...
      while (status === "running" || status === "notStarted") {
        ...
        if (status === "succeeded") {
          ...
          // Utilisez plusieurs noms possibles pour chaque champ
          const montantHT = fields.SubTotal?.valueString || fields["Total HT"]?.valueString || "";
          const tva = fields.TotalTax?.valueString || fields["Total TVA"]?.valueString || "";
          const fodec = fields.FODEC?.valueString || fields["FODEC"]?.valueString || "";
          const timbre = fields.Timbre?.valueString || fields["Timbre"]?.valueString || "";
          const montantTotal = fields.InvoiceTotal?.valueString || fields["Net A Payer"]?.valueString || "";
          ...
          Swal.fire("Succès", "Les données de la facture ont été extraites avec succès !", "success");
          break;
        }
  ...
  //             date_facture: fields.InvoiceDate?.valueDate || "",
  //             num_facture: fields.InvoiceId?.valueString || "",
  //             montant_HT_facture: fields["SubTotal"]?.valueString || "", 
  //             TVA_facture: fields["TotalTax"]?.valueString || "", 
  //             FODEC_sur_facture: fields["FODEC"]?.valueString || "",
  //             timbre_facture: fields["Timbre"]?.valueString || "",
  //             montant_total_facture: fields["InvoiceTotal"]?.valueString || "", 
  //           });
            

  //         console.log(resultResponse.data.analyzeResult.documentResults[0].fields);
