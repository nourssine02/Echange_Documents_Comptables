...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  //       "https://api-inference.huggingface.co/models/bert-base-uncased",  // Assurez-vous que cette URL est correcte pour le modèle que vous utilisez
  //       {
  //         inputs: text,
  //       },
  //       {
  ...
  const fetchAICompletion = async (text) => {
    try {
      const response = await axios.post(
        ...
        {
          headers: {
            Authorization: `Bearer YOUR_HUGGINGFACE_API_TOKEN`,  // Remplacez par votre token d'autorisation Hugging Face
          },
        }
      );
