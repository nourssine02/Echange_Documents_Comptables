...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  const structureInfoWithChatGPT = async (extractedText) => {
    ...
    if (chatGPTData) {
      ...
      lines.forEach((line) => {
        if (line.startsWith("Date:") || line.startsWith("Le") )
          date_facture = line.replace("Date:", "").replace("Le", "").trim();
        if (line.startsWith("Numero:") || line.startsWith("Nº")) {
          num_facture = line.replace("Numero:", "").replace("Nº", "").trim();
        }       
