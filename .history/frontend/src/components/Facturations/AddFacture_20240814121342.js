...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  const handleOCRClick = async (file) => {
  ...
  //     setFacture({
  //       ...facture,
  //       num_facture: extractedText.match(/Numéro de Facture: (\S+)/)?.[1] || '',
  //       date_facture: extractedText.match(/Date: (\d{2}\/\d{2}\/\d{4})/)?.[1] || '',
  //       montant_HT_facture: extractedText.match(/Montant HT: (\d+,\d+)/)?.[1] || '',
  //       TVA_facture: extractedText.match(/TVA: (\d+,\d+)/)?.[1] || '',
  //       montant_total_facture: extractedText.match(/Total: (\d+,\d+)/)?.[1] || '',
  //     });
  
  //     Swal.fire({
  //       icon: "success",
  //       title: "Succès",
  //       text: "OCR traitée avec succès !",
  //     });
  //   } catch (error) {
