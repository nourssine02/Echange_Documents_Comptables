...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      ...
      reader.onload = () => {
        ...
        setFacture((prev) => ({
          ...prev,
          document_fichier: base64Data, // Stocke uniquement la partie base64 des données du fichier
        }));
      };
      reader.readAsDataURL(selectedFile); // Lecture du fichier sous forme d'URL de données
    } else {
      setFacture((prev) => ({ ...prev, document_fichier: "" })); // Si aucun fichier, met à jour le champ avec une chaîne vide
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateAllFields()) {
      ...
      try {
        // Envoi de la facture
        const token = localStorage.getItem("token");
        const response = await axios.post("http://localhost:5000/facture", data, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'multipart/form-data', 
          }
        ...
        // Ajouter la notification après l'ajout réussi de la facture
        const notificationMessage = `${user.identite} a ajouté une nouvelle facture`;
  
        // Vérifier si c'est bien un comptable, sinon ajuster la condition en fonction des besoins
        if (user.role === "comptable") {
          // Notification data structure
          const notificationData = {
            userId: user.id, // Identifiant de l'utilisateur (comptable)
  ...
  <input
    ...
    onChange={handleFileChange} // Correction : on utilise handleFileChange ici
  />
