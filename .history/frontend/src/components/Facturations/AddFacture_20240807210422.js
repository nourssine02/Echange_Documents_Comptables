  ...
  const handleOCR = async (base64Image) => {
    try {
      ...
      setFacture((prev) => ({
        ...
        montant_total_facture: data.amount || prev.montant_total_facture,
        montant_HT_facture: data.amountHT || prev.montant_HT_facture,
        TVA_facture: data.tva || prev.TVA_facture,
        timbre_facture: data.stamp || prev.timbre_facture,
        FODEC_sur_facture: data.fodec || prev.FODEC_sur_facture,
        montant_total_facture: data.totalAmount || prev.autre_montant_facture
      }));
      Swal.fire({
        icon: "success",
        title: "Succès",
