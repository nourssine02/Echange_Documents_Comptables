...
const structureInfoWithAI = async (extractedText) => {
  try {
    // Utilisation du modèle LayoutLMv3 pour factures
    const response = await hf.tokenClassification({
      model: "microsoft/layoutlmv3-base", // Modèle spécialisé pour la mise en page de documents
      inputs: extractedText,
    });

    const aiData = {};

    // Analyse des entités extraites par LayoutLMv3
    response.forEach((entity) => {
      if (entity.entity_group === "DATE") {
        aiData.date_facture = entity.word; // Identification automatique de la date
      } 
      else if (entity.entity_group === "INVOICE_NUMBER") {
        aiData.num_facture = entity.word; // Identification automatique du numéro de facture
      } 
      else if (entity.entity_group === "TOTAL_AMOUNT") {
        aiData.montant_total_facture = entity.word.replace("DT", "").trim(); // Extraction du montant total
      } 
      else if (entity.entity_group === "SUBTOTAL") {
        aiData.montant_HT_facture = entity.word.replace("DT", "").trim(); // Extraction du montant HT
      } 
      else if (entity.entity_group === "FODEC") {
        aiData.FODEC_sur_facture = entity.word.replace("DT", "").trim(); // Extraction de FODEC
      } 
      else if (entity.entity_group === "TVA") {
        aiData.TVA_facture = entity.word.replace("DT", "").trim(); // Extraction de TVA
      } 
      else if (entity.entity_group === "STAMP") {
        aiData.timbre_facture = entity.word.replace("DT", "").trim(); // Extraction du timbre (taxe)
      }
    });
