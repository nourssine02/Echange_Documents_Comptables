...
const AddFacture = () => {
  ...
  const handleChangeArticle = (selectedOption, index) => {
    ...
    updatedFamilles[index].prix = selectedOption.prix; // Assuming the price is included in the selected option
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        ...
        setArticles(
          res.data.map((article) => ({ value: article.article, label: article.article, prix: article.prix }))
        );
      } catch (err) {
        console.log(err);
      }
