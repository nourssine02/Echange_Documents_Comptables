...
const AddFacture = ({ isSidebarOpen }) => {
  ...
  const handleOCRClick = async () => {
    ...
    data.append('document_fichier', file);
    ...
    try {
      ...
      // Vous devrez peut-être utiliser des expressions régulières ou une autre logique
      // pour extraire les différentes informations du texte brut retourné par Tesseract
      // et les insérer dans les champs correspondants du formulaire.
      
      // Exemple de remplissage des champs
      setFormData({
        numeroFacture: extractedText.match(/Numéro de Facture: (\S+)/)?.[1] || '',
        dateFacture: extractedText.match(/Date: (\d{2}\/\d{2}\/\d{4})/)?.[1] || '',
        montantHT: extractedText.match(/Montant HT: (\d+,\d+)/)?.[1] || '',
        tva: extractedText.match(/TVA: (\d+,\d+)/)?.[1] || '',
        montantTotal: extractedText.match(/Total: (\d+,\d+)/)?.[1] || '',
      });
