...
const structureInfoWithAI = async (extractedText) => {
  try {
    // Utilisation du modèle NER pour extraire les entités
    const response = await hf.tokenClassification({
      model: "dslim/bert-base-NER", // Modèle NER pour l'extraction d'entités
      inputs: extractedText,
    });

    const aiData = {};

    // Analyse des entités extraites par le modèle NER
    response.forEach((entity) => {
      if (entity.entity_group === "DATE") {
        ...
        aiData.timbre_facture = entity.word.replace("DT", "").trim(); // Extraction du timbre
      }
