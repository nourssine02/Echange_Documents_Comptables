...
const Configurations = () => {
  ...
  const fetchTaxRates = async () => {
    try {
      const response = await axios.get("http://localhost:5000/tax-rates");
  ...
  const addTaxRate = async () => {
    try {
      await axios.post("http://localhost:3001/taux_retenue_source", {
        rate: newRate,
      });
      setNewRate("");
      fetchTaxRates();
  ...
  const toggleTaxRate = async (id, active) => {
    try {
      await axios.put(`http://localhost:3001/taux_retenue_source/${id}`, {
        active: !active,
      });
      fetchTaxRates();
    } catch (error) {
