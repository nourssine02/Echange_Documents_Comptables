...
import notificationSound from "../assets/notification.mp3"; // Assurez-vous d'avoir un fichier son à cet emplacement
  ...
  const fetchNotifications = async () => {
    if (user) {
      try {
        ...
        setUnreadNotifications(res.data.filter(notification => !notification.read));
        ...
        <ul className="navbar-nav navbar-nav-right">
          <li className="nav-item dropdown">
            ...
            <div className={`dropdown-menu dropdown-menu-right navbar-dropdown ${isNotificationDropdownOpen ? "show" : ""}`} aria-labelledby="notificationDropdown">
              <h6 className="dropdown-header">Notifications ({unreadNotifications.length})</h6>
