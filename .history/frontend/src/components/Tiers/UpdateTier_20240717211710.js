...
const UpdateTier = ({ isSidebarOpen }) => {
  ...
  const validateForm = () => {
    ...
    // Validation logic
    if (!tier.code_tiers) {
      errors.code_tiers = "Le code tiers est requis.";
      isValid = false;
    }
    if (!tier.date_creation) {
    ...
    if (tier.type === "Autre" && !tier.autreType) {
      errors.autreType = "Le type 'Autre' est requis.";
      isValid = false;
    }
    if (!tier.identite) {
      errors.identite = "L'identité est requise.";
      isValid = false;
    }
    if (!tier.MF_CIN) {
      errors.MF_CIN = "Le MF/CIN est requis.";
      isValid = false;
    }
    if (!tier.tel) {
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`/tiers/${id}`, tier).then((response) => {
      Swal.fire({
        icon: "success",
        title: "Succès",
