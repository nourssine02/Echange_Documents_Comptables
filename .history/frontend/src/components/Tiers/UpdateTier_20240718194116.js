...
const UpdateTier = ({ isSidebarOpen }) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`http://localhost:5000/tiers/${id}`, tier).then((response) => {
      Swal.fire({
        icon: "success",
        title: "Succès",
        ...
        setTier((prev) => ({
          ...
          banques: Array.isArray(data.banques)
            ? data.banques.map((banque) => ({
                value: banque.id, // Assuming banque.id is the banque_id
                label: banquesOptions.find(option => option.value === banque.id)?.label || banque.id,
              }))
            : [],
        }));
      } catch (err) {
