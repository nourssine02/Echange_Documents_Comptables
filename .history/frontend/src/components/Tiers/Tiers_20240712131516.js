...
const Tiers = ({ isSidebarOpen }) => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      // Mettre à jour l'état local ou recharger la liste des tiers
    } catch (err) {
      if (err.response && err.response.data.error) {
        // Gestion des messages d'erreur détaillés pour l'utilisateur
        if (err.response.status === 400) {
          toast.error(err.response.data.error); // Message détaillé des dépendances
        } else {
          toast.error("Le tier ne peut pas être supprimé car il est associé à des enregistrements dans les tables.");
        }
      } else {
