...
const UpdateTier = ({ isSidebarOpen }) => {
  ...
  const handleClick = async (e) => {
    ...
    if (formValid && !hasErrors) {
      try {
        const token = localStorage.getItem('token'); // Récupérer le token du stockage local ou d'une autre source
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        const data = res.data; // Assuming data is not wrapped in an array based on previous API response handling
        if (!data) {
      ...
      .catch((err) => console.error("Error fetching tier data:", err));
  }, [id]);
