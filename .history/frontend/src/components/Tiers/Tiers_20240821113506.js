...
const Tiers = ({ isSidebarOpen }) => {
  ...
  const [banques, setBanques] = useState({});  // Pour stocker les banques de chaque tier
  ...
  // Récupérer les banques associées à un tier
  const fetchBanques = async (tierId) => {
    try {
      const res = await axios.get(`http://localhost:5000/tiers/${tierId}/banques`);
      setBanques((prevBanques) => ({
        ...prevBanques,
        [tierId]: res.data, // Stocker les banques pour le tier spécifié
      }));
    } catch (err) {
      console.log(err);
    }
                        ...
                        <tr key={tier.id}>
                          ...
                          <td>
                            ...
                            <button
                              ...
                              onClick={() => fetchBanques(tier.id)}
                            >
                              + Afficher Banques
                            </button>
                            {banques[tier.id] && (
                              <ul>
                                {banques[tier.id].map((banque) => (
                                  <li key={banque.id}>{banque.name}</li>
                                ))}
                              </ul>
                            )}
                          </td>
