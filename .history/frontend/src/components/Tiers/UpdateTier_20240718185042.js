...
const UpdateTier = ({ isSidebarOpen }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const tierToSend = {
      ...tier,
      banques: tier.banques.map(banque => banque.value), // Extraire uniquement les IDs des banques
    };
  
    axios.put(`http://localhost:5000/tiers/${id}`, tierToSend).then((response) => {
      Swal.fire({
        icon: "success",
        title: "Succès",
    ...
    const fetchTier = async () => {
      try {
        ...
        setTier((prev) => ({
          ...
          ? data.banques.map((banqueId) => banquesOptions.find(banque => banque.value === banqueId))
          : [],
        }));
      } catch (err) {
