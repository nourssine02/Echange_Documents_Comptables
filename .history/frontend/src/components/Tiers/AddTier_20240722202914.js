    ...
    if (!hasErrors) {
      ...
      try {
        ...
        if (user.role === "comptable" || user.role === "client") {
          ...
          const notificationData = { userId: user.id, message: notificationMessage };
  
          await axiosInstance.post("http://localhost:5000/notifications", notificationData);
        }
  
        toast.success("Tier ajouté avec succès !");
        //setAlert({ type: "success", message: "Tier ajouté avec succès" });
        Swal.fire({
          icon: "success",
          title: "Succès",
        ...
        console.log(err);
        //setAlert({ type: "danger", message: "Erreur lors de l'ajout du tier." });
        Swal.fire({
          icon: "error",
          title: "Erreur",
