...
const UpdateTier = () => {
      ...
      try {
        ...
        setTier((prev) => ({
          ...
          banques: Array.isArray(data.banques)
            ? data.banques.map(banque => ({ value: banque.id, label: banque.name })) // Example mapping logic
            : []
        }));
      } catch (err) {
  ...
  const handleMultiSelectChange = (selectedOptions) => {
    setTier((prev) => ({ ...prev, banques: selectedOptions.map(option => option.value) }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/tiers/${id}`, tier);
      Swal.fire({
        icon: "success",
        title: "Success",
            ...
            <form className="forms-sample"  onSubmit={handleSubmit}>
              <div className="row">
                ...
                <div className="col-md-4">
                  ...
                  value={tier.banques.map(banque => ({ value: banque.id, label: banque.name }))} // Example value mapping
                />
